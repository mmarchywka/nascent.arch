#ifndef MJM_TCL_NUMERIC_H__
#define MJM_TCL_NUMERIC_H__
...
*/
// class mjm_lorentzian_kernel;

class mjm_kernel
{
typedef mjm_kernel Myt;
protected:
...
public:
static Myt * make(StrTy & name,const D param); //  {
// return new mjm_lorentzian_kernel(param); // should have global means to find..
// return  new mjm_kernel(); 
//}
static Myt * make(StrTy & name) { return  new mjm_kernel(); }
virtual  D eval( const V & s, const D & t) const 
{
return 0; 
...
class mjm_lorentzian_kernel : public  mjm_kernel
{
typedef mjm_lorentzian_kernel Myt;
typedef mjm_kernel Super;
D m_width;
mutable IdxTy m_size, m_last_first, m_last_last;
mutable IdxTy m_calls;
public:
mjm_lorentzian_kernel(const D & x) : m_width(x),m_size(0),
m_last_first(0), m_last_last(0),m_calls(0) {
std::cout<<MM_MARK<<" making lorentz withg w="<<m_width<<CRLF;

}

virtual  D eval( const V & s, const D & t) const 
{
std::cout<<MM_MARK<<" t="<<t<<CRLF;
m_size=s.size();
D tog=0;
for(IdxTy i=0; i<m_size; ++i)
{
D d=s[i][0]-t;
tog+=s[i][1]/(m_width+(d*d));
...
//mjm_kernel * mjm_kernel::make(StrTy & name,const D param) {
//return new mjm_lorentzian_kernel(param); // should have global means to find..
// return  new mjm_kernel(); 
//}
/*
...
std::cout<<MM_MARK<<" loading "<<current()<<" from "<<p[ORIGIN]<<CRLF;
IsTy * is = mjm_io::get_in(p[ORIGIN].c_str(),std::cout);
m->load(is);
...
// output name, kernel, start, delta, end, kernel param
static int regular(CdTy cd, InTy * in, I objc, ArTy objv[])
{
TCL_PRERAMBLE_SHORT
SigTy * m= & sigs()[current()];
SigTy * mout= & sigs()[p[ORIGIN]];
std::cout<<MM_MARK<<" smoothing and sampling to  "<<current()<<
" to "<<p[ORIGIN]<< " flags " <<v[1]<<CRLF;
const IdxTy a=2;
const IdxTy vs=v.size();
typedef mjm_kernel K;
K * kernel ;
if ( vs>a+3)  kernel=K::make(p[ORIGIN+1], v[a+3]);
else  kernel=K::make(p[ORIGIN+1], 0);
std::cout<<MM_MARK<<" smoothing and sampling to  "<<v[a]<<" "<<v[a+1]<<" "<<v[a+2]<<CRLF;
m->regular(*mout,*kernel,v[a],v[a+1],v[a+2]);
...
Tcl_CreateObjCommand(tcl_interp, "num_set", set,0,0);
//Tcl_CreateObjCommand(tcl_interp, "dirac_store", store,0,0);
Tcl_CreateObjCommand(tcl_interp, "num_store", store,0,0);
Tcl_CreateObjCommand(tcl_interp, "num_load", load,0,0);
Tcl_CreateObjCommand(tcl_interp, "num_regular", regular,0,0);
