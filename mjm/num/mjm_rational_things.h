...
const bool ratio_flag=(style&(1<<8));
RatCo nest;
nest.push_back(1);
for(IdxTy i=1; i<=n; ++i)
{
...
std::cout<<format_poly(i,n,nest,style&255)<<CRLF;
if (ratio_flag)
{
const RatTy one=RatTy(1);
Ss ss; 
MM_SZ_LOOP(i,nest,nsz)
{
const RatTy & x1=nest[i];
const RatTy & x2=(i==0)?one:nest[i-1];
RatTy rat=x2/x1;
rat.simplify();
ss<<rat<<" ";
}
...
MM_LOOP(ii,c) sum+=(*ii);
if (sum>L) 
{ MM_ERR(" danger will robinson sizes wrong "<<MMPR3(c.size(),sum,L)) }
...
MM_ERR(" making cache "<<MMPR3(cache.size(),L,sum))
for(IdxTy i=0; i<cache.size(); ++i) 
	cache[i]=(simplify(fix_f(sum_coefs(i,1))));
MM_ERR(" done making cache "<<MMPR3(total,offset,n))
...
void eval_poly(RatTy & v, const RatCo & d,const IdxTy n)
{
RatTy x=RatTy(n);
RatTy x2=RatTy(n*n);
const IdxTy sz=d.size();
// no zero term anyway; 
...
const RatTy & tot=P[P.size()-1];
for(IdxTy i=0; i<10; ++i)
{
MM_ERR(MMPR3(i,p[i],P[i])<<MMPR2(D(p[i]/tot),D(P[i]/tot)))
}
// P(o)+p(o)*(l-o-1)
const IdxTy l1=c[0];
const IdxTy l2=c[1];
const IdxTy posz=l1+l2;
D pot[posz];
memset(pot,0,posz*sizeof(D));
// these are EFFECTIVE amounts not actual 
for(IdxTy o1=0; o1<l1; ++o1)
{
for(IdxTy o2=0; o2<l2; ++o2)
{
const IdxTy o=o1+o2;
RatTy po1=(tot-P[o1]+p[o1]+RatTy(l1-o1-1)*p[o1])/tot;
RatTy po2=(tot-P[o2]+p[o2]+RatTy(l1-o2-1)*p[o2])/tot;
po1.simplify();
po2.simplify();
//const RatTy po1=(P[o1+1]+RatTy(l1-o1-1)*p[o1])/tot;
//const RatTy po2=(P[o2+1]+RatTy(l2-o2-1)*p[o2])/tot;
pot[o]+=D(po1*po2);
MM_ERR(MMPR4(o,o1,o2,D(po1))<<MMPR4(D(po2),D(po1*po2),P[o1],p[o1]))
}
}
D totp=0;
D denom=(l1-0)*(l2-0);
for (IdxTy i=0; i<posz; ++i) {totp+=pot[i]; MM_ERR(MMPR4(i,pot[i],totp,(totp/denom))) }


memset(pot,0,posz*sizeof(D));
// this could be zero due to total obscuration 
// this is now the EFFECTIVE amount of string1 REVEALED
// NOT the actual amount beyond edge of island. 
for(IdxTy o1=0; o1<l1; ++o1)
{
// in the other code, this is P>= and the only term needed is P
RatTy po1=(tot-P[o1]+p[o1]+RatTy(l1-o1-1)*p[o1])/tot;
po1.simplify();
pot[o1]+=D(po1);
MM_ERR(MMPR3(o1,o1,D(po1))<<MMPR4(pot[o1],D(po1),P[o1],p[o1]))

} // o1
 totp=0;
// this is now the full length as zero is allowed.  
 denom=(l1-1*0);
for (IdxTy i=0; i<posz; ++i) {totp+=pot[i]; MM_ERR(MMPR4(i,pot[i],totp,(totp/denom))) }
