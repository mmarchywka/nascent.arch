#ifndef MJM_FICK_EQUATIONS_H__
#define MJM_FICK_EQUATIONS_H__

#if 0
// "CR" enum is fuxked with my CR macro FUDD 
...
class fleqn_params : public mjm_logic_base
{
...
typedef Tr::Ss Ss;
public:
fleqn_params( const StrTy & nm) : Super(nm) {}
fleqn_params() : Super() {}
...
class fleqn_tree : public mjm_logic_base
{
typedef mjm_logic_base Super;
typedef Super::Tr Tr;
typedef  Tr::D D;
...
typedef Tr::Ss Ss;
public:
fleqn_tree( const StrTy & nm) : Super(nm) {}
fleqn_tree() : Super() {}
//IdxTy output_offset() const { return m_map.get_uint("output_offset",output_freq()-1); } // freq-1;
//StrTy output_label() const { return m_map.get_string("output_label","fick"); }
//D Vt() const { return m_map.get_double("Vt",.0259); }
...
//template <class Ty> 
class mjm_fick_es : public EquationSystems
{
typedef EquationSystems Super;
typedef mjm_fick_es Myt;
...
{
MM_ERR(" Create mjm_fick_es")

}
  	static void Destroy(Myt ** b)
	{
MM_ERR(" Destroy  mjm_fick_es")

}

	//mjm_nonlin_es(UnstructuredMesh & m)
template <class FUDD> 	mjm_fick_es(FUDD & m)
: Super(m)
{
//MM_ERR(" Ctor  mjm_fick_es")
//this fails because one system has to be added ibefore
//this->EquationSystems::init();
}
  ~mjm_fick_es()
  {
//MM_ERR(" Dtor  mjm_fick_es")
...
//template <class Ty> 
class mjm_fick_jr : public NonlinearImplicitSystem,
...
mjm_fick_jr(EquationSystems & eqSys,
                   const StrTy  & name_in,
                 const IdxTy number_in) :
	NonlinearImplicitSystem(eqSys, name_in, number_in)
	,m_es(dynamic_cast<EsTy &>(eqSys)),mp_lis(0)
	, m_cm(&m_cm_default) {m_mesh=& get_mesh();}
...
void set_system( /* const */ MySparse & A, const MyBlock & b, const bool expand=false)
{
...
if ( expand)
{
libMesh::PetscMatrix<Real>*  Ax=(libMesh::PetscMatrix<Real>*)(matrix);
MatSetOption(Ax->mat(), MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE)   ;
}
   	...
   	while (si)
  	{
    	const IdxTy i=si.i();
    	const IdxTy j=si.j();
	//	MM_MSG(MMPR3(i,j,A(i,j)))
   		(*matrix).set(i,j,A(i,j));	 
		++si;
 	}

	for (IdxTy i=0; i<b.size(); ++i) (*rhs).set(i,b(i));

}
void mjm_linear_solve(const bool expand=false,const Real &  rtol=1e-10, const IdxTy iters=500)
{
...
//MM_MSG(" doing linear solver attempt")
...
// was 1e-18 and 5000 but too slow 
//MM_ERR(" call solver ")
//p->solve(*matrix,0,*solution,*rhs,1e-18,5000);
//p->solve(*matrix,0,*solution,*rhs,rtol,iters);
p->solve(*matrix,*solution,*rhs,rtol,iters);
//MM_ERR(" return from  solver ")
m_reason=p->get_converged_reason();
//MM_ERR(" got code  solver "<<m_reason<<" "<<converged_string(m_reason))
//MM_MSG(" got code  solver "<<m_reason<<" "<<converged_string(m_reason))

delete p; 
}
bool diverged() const { return m_reason<0; } 
bool may_be_ok() const { return (!diverged()) || (m_reason==libMesh::DIVERGED_ITS) ;} 
int get_converged_reason() const { return m_reason; }
StrTy converged_string(const int x) const 
{
StrTy y;
//cat asdf | awk '{print "if (x==libMesh::"$1") { y=\""$1"\"; return;}"}'
if (x==libMesh::CONVERGED_RTOL_NORMAL) { y="CONVERGED_RTOL_NORMAL"; return y;}
if (x==libMesh::CONVERGED_ATOL_NORMAL) { y="CONVERGED_ATOL_NORMAL"; return y;}
if (x==libMesh::CONVERGED_RTOL) { y="CONVERGED_RTOL"; return y;}
if (x==libMesh::CONVERGED_ATOL) { y="CONVERGED_ATOL"; return y;}
if (x==libMesh::CONVERGED_ITS) { y="CONVERGED_ITS"; return y;}
if (x==libMesh::CONVERGED_CG_NEG_CURVE) { y="CONVERGED_CG_NEG_CURVE"; return y;}
if (x==libMesh::CONVERGED_CG_CONSTRAINED) { y="CONVERGED_CG_CONSTRAINED"; return y;}
if (x==libMesh::CONVERGED_STEP_LENGTH) { y="CONVERGED_STEP_LENGTH"; return y;}
if (x==libMesh::CONVERGED_HAPPY_BREAKDOWN) { y="CONVERGED_HAPPY_BREAKDOWN"; return y;}
if (x==libMesh::DIVERGED_NULL) { y="DIVERGED_NULL"; return y;}
if (x==libMesh::DIVERGED_ITS) { y="DIVERGED_ITS"; return y;}
if (x==libMesh::DIVERGED_DTOL) { y="DIVERGED_DTOL"; return y;}
if (x==libMesh::DIVERGED_BREAKDOWN) { y="DIVERGED_BREAKDOWN"; return y;}
if (x==libMesh::DIVERGED_BREAKDOWN_BICG) { y="DIVERGED_BREAKDOWN_BICG"; return y;}
if (x==libMesh::DIVERGED_NONSYMMETRIC) { y="DIVERGED_NONSYMMETRIC"; return y;}
if (x==libMesh::DIVERGED_INDEFINITE_PC) { y="DIVERGED_INDEFINITE_PC"; return y;}
if (x==libMesh::DIVERGED_NAN) { y="DIVERGED_NAN"; return y;}
if (x==libMesh::DIVERGED_INDEFINITE_MAT) { y="DIVERGED_INDEFINITE_MAT"; return y;}
//if (x==libMesh::DIVERGED_PCSETUP_FAILED) { y="DIVERGED_PCSETUP_FAILED"; return y;}
if (x==libMesh::CONVERGED_ITERATING) { y="CONVERGED_ITERATING"; return y;}
if (x==libMesh::UNKNOWN_FLAG) { y="UNKNOWN_FLAG"; return y;}

return StrTy("Danger Will Robinson bad code");
}
...
void DumpSolution()
{
// this has no ng effect, the only valid thing is a zero k 
...
// would work but need the mesh... 
mjm_libmesh_util::DumpVectorHiRes(std::cout, *m_mesh, (*solution), "SolutionDump");
mjm_libmesh_util::DumpActivePolygons(std::cout, *m_mesh, (*solution), "Poly");
}
// vars must be in consecutive columns
void copy_solution(MyBlock & d,const IdxTy col, const IdxTy group=0) //const
{
// sioln vector is indexed by DOF, the matrix is indexed by node number or id. 
// so this does not work, need a node iterator 
typedef mjm_libmesh_util::MeshNodeItor<true> NodeItor; 
NodeItor mi(*m_mesh,group);
while (mi.ok())
{
	const IdxTy i=mi.dof();
	const IdxTy id=mi.id();
	const IdxTy var=mi.var();
//MM_MSG(" test copy itor "<<MMPR4(i,id,var,(*solution)(i)))
	d(id,col+var)=(*solution)(i);
	mi.incvar();
}

}
void blend_solution(MyBlock & d,const IdxTy col, const Real  & alpha, const IdxTy group=0, const bool flip=false ) //const
{
// sioln vector is indexed by DOF, the matrix is indexed by node number or id. 
// so this does not work, need a node iterator 
typedef mjm_libmesh_util::MeshNodeItor<true> NodeItor; 
...
while (mi.ok())
{
	...
	const IdxTy id=mi.id();
	const IdxTy var=mi.var();
	const Real  dijb=beta*d(id,col+var);
	Real as=alpha*(*solution)(i);
if (flip) if (as<0) as=0; // -1.0/as;	
//MM_MSG(" test copy itor "<<MMPR4(i,id,var,(*solution)(i)))
	//d(id,col+var)=alpha*(*solution)(i);
	//d(id,col+var)=alpha*(*solution)(i)+beta*dij;
	//d(id,col+var)=alpha*(*solution)(i)+beta*dij;
	d(id,col+var)=dijb+as;
	mi.incvar();
}
...
// it seems to compute but dump fails on dof ???
get_dof_map(); //???????
...
// this only ing workse where it will get wiped out later FUDD 
//project_solution(Myt::init_value<MLATy>, libmesh_nullptr, m_es.parameters);
// this AK compiles but the one that shouldwork does not k
...
if ((&(m_mla->mesh)) !=(&(m_es.get_mesh()))) MM_ERR(" mesh mismatch shot")
...
// this a has no king effect everythihng is zero ck 
project_solution(Myt::init_value<MLATy>, libmesh_nullptr, m_es.parameters);
...
// works but needs ing mesh 
mjm_libmesh_util::DumpVector(std::cerr, *m_mesh, (*R), StrTy("res")+ival);
}
...
template <class Tv, class Ts,class Tfck> 
	void zero_assembly( Tv & residual, Ts * jacobian, const Tfck & sol)
{
	(*m_cm).mark("assbegin");
...
// this is probably the beginning of the thing 
template <class Tv, class Ts,class Tfck> void assembly( Tv * R, Ts * J, const Tfck & sol)
{
#if 0
	(*m_cm).mark("assbegin");
...
void do_dump(const bool do_or_no, const To & x, const Tl&  label)
{
	...
	if (ival.length()==11) ival=StrTy("iterations=0");
// works needs mesh 
	mjm_libmesh_util::DumpVector(std::cerr, *m_mesh, x, StrTy(label)+ival);
}
...
//if (m_zero!=0)
#if 0 
template <class Tv, class Ts,class Tuck> void assembly_converting_to_mm( Tv * R, Ts * J, const Tuck & sol)
{
	(*m_cm).mark("assbegin");
    static int ass_count=0;
    MM_MSG(" ass_count="<<ass_count)
	...
	//		{ MM_MSG(" rhs_block_zed(0,0)= "<<rhs_block_zed(0,0)) } 
		...
		if (doing_j)
		{
			...
			//	Real deltaeff=xzed*.0001;
				...
				my_sol(dofj)+=deltaeff;
				const Real uckv=(*solp)(dofj); // there was some reason not to 
...
//		if (rhs_block(0,0)!=0) { MM_MSG(" rhs_block(0,0)= "<<rhs_block(0,0)) } 
				...
				solp->add(dofj,oldv);
				my_sol(dofj)=my_xzed;
				// whty the FUDD does this not ng work if add fudding workds
				//(*solp).set(dofj,oldv);
//				solp->el(dofj)=oldv;
// wrong state wtf	
//			solp->set(dofj,xzed);
...
template <class Tv, class Ts,class Tuck> void assembly_working( Tv * R, Ts * J, const Tuck & sol)
{
#if 0
	(*m_cm).mark("assbegin");
    static int ass_count=0;
    ...
    for ( ; el != end_el ; ++el)
    {
        ...
        //if (elem->has_children()) continue; 
        mla.re_init(elem);
//   const Real  x=point_value(0,*(*elem).get_node(0),*elem);
//		MM_ERR(" fk the dof akkkkk "<<xfut)
...
#if 0
template <class Tv, class Ts,class Tfck> void assembly_old( Tv * R, Ts * J, const Tfck & sol)
{
	(*m_cm).mark("assbegin");
    static int ass_count=0;
    MM_MSG(" ass_count="<<ass_count)
    ...
    for ( ; el != end_el ; ++el)
    {
        ...
        //if (elem->has_children()) continue; 
        mla.re_init(elem);
//   const Real  xfcksht=point_value(0,*(*elem).get_node(0),*elem);
//		MM_ERR(" the dof ass  fuddkkkkk "<<xfcksht)
        mjm_libmesh_util::ReInitMats(Ke_var, Fe_var,Ke,Fe,mla.di);
		const IdxTy dms[]={mla.nvar,mla.nvar,mla.di.n_var_dofs,mla.di.n_var_dofs};
		const IdxTy dmsr[]={mla.nvar,mla.di.n_var_dofs};
		MyBlock rhs_block(MyBlock::loc(dmsr,2));
...
//Number ASSFUDD=-111;
//return ASSFUDD; 
//MM_ERR(" ack init ing called ")
    // we could just make a global instance 
    //return 0; 
// none of this t compiles fudd
