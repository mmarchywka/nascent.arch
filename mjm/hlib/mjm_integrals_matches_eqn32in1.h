...
template<class Td>  void next_gauss_der_polynomial(Td & dest, const Td & v1, const double & scale=1.0)
{
const IdxTy sz1=v1.size();
if (sz1<1) return; 
...
static void burmann_ref1_eq21(Tvec & dest, Tf & func, const D & z0, const IdxTy mu, const IdxTy nu, const IdxTy kmax, const IdxTy flags=0)
{
//const D z0=0;
...
static void burmann_ref1_eq14(Tvec & dest, Tf & func, Tpf & phifunc, Tpf2 & phistarfunc, const D & z0, const IdxTy nu, const IdxTy nmax, const IdxTy flags=0)
{
...
dense_2_polynomial(const Super & m ): Super(m),m_n(m.size()) {}
...
static void integrate_e( Myt & dexp,Myt & derf, const Myt & x, const value_type & b, const IdxTy factor, const IdxTy dir)
{
 mjm_integrals mi;
...
template<class Tx, class Ty, class Tn > bool  add_term( Tx & terms, Ty & ss, const Tn & t) const 
{
	if (t!=0) { if (terms!=0) { if ( t>0) ss<<"+"; } ss<<t; ++terms; return true;  }
	return false; 
}
...
for (IdxTy i=0; i<dest.size(); ++i) { MM_MSG("bermann erf "<<MMPR2(i,dest[i])) }
