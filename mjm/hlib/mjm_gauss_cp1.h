...
MM_MSG(MMPR3(beta,dest,rescomp)<<MMPR3((respoly-reses),respoly,reses))
...
//integrate_e( dexp,derf,  x,  b,factor,  dir)
	D2::integrate_e_new(yexp,yerf,yint,beta,1,0);
...
qint=temp;
	...
	if (debug_step_2) { MM_MSG(" final integrand as f of y and x " ) } 
	if (debug_step_2) { MM_MSG(MMPR(yexp.to_string(0,"y","x"))) } 
	if (debug_step_2) { MM_MSG(MMPR(yerf.to_string(0,"y","x"))) } 
...
*/
	...
	if (debug_step_3) { MM_MSG(MMPR(yexp2.to_string())) } 
	if (debug_step_3) { MM_MSG(MMPR(yerf2.to_string())) } 
	...
	const D r2=yerf.evaluate(yff,sf.xu)*erf(bsyff);
	const D r3=yexp.evaluate(yii,sf.xl)*exp(-beta*yii*yii);
	const D r4=yerf.evaluate(yii,sf.xl)*erf(bsyii);

	const D r5=yexp.evaluate(yfi,sf.xu)*exp(-beta*yfi*yfi);
	const D r6=yerf.evaluate(yfi,sf.xu)*erf(bsyfi);
	const D r7=yexp.evaluate(yif,sf.xl)*exp(-beta*yif*yif);
	const D r8=yerf.evaluate(yif,sf.xl)*erf(bsyif);
	
	res+= r1+r2+r3+r4-r5-r6-r7-r8;	
	MM_MSG(MMPR(res)<<MMPR4(r1,r2,r3,r4)<<MMPR4(r5,r6,r7,r8))
