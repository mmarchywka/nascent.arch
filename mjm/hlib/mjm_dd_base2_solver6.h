...
,solver(0), non_linear_fac(1),maxstep(20),minstep(-20),maxnn(1e20)
 {}
...
for ( IdxTy i=0; i<szdiag; ++i) diags[i]=0;
for ( IdxTy i=0; i<szn; ++i) rhs[i]=0;
set_pointers ();
...
~blas_diags() { delete [] rhs; delete [] diags;delete[] diagsfudd; } 
...
// create tri-diagonal by using the target current/flux to 
// remove e-field. 
...
const IdxTy sz=g.size();
if ( sz==0) return; 
const bool dump_terms=!true;
const bool dump_solution=!true;
...
// hard to tell with 1000 points and the - one blows up soon. 
//const Value flux=1e15;
// 1e16 works fwd bias,  .72 volts or so but more dies 
//const Value flux=1e16;
//const Value flux=-1e15;
const Value flux=0;
...
for(IdxTy i=end2; i<szn; ++i){ diag[i]=diagb; rhs[i]=0;}
...
// oh crap, these are not right lol 
//const Value gradx=dxi2*(px[i+1]-px[i-1]);
//const Value grady=dxi2*(nx[i+1]-nx[i-1]);
//const Value lapx=(px[i+1]+px[i-1]-2.0*p)*dxfi;
//const Value lapy=(nx[i+1]+nx[i-1]-2.0*n)*dxfi;
...
//const Value diagb=1e28;
for(IdxTy i=0; i<starter; ++i){ diag[i]=diag[starter]; rhs[i]=0;}
for(IdxTy i=end2; i<szn; ++i){ diag[i]=diag[end2-1]; rhs[i]=0;}
...
/*
dgtsv_(&N,&NRHS,&subdiag[0],&diag[0],&superdiag[0],&rhs[0],&LDB,&INFO);
if (INFO!=0) { { MM_MSG(siter<<" DGTSV error code is "<<INFO) } }
IdxTy j=0;
*/
//delete diagsfudd;
delete [] ipiv; 
...
// 1 M data points. 
static void update_carriers_4( MatSt & m, const MatPr & pr, const GP & gp, const sim_flags & flags, JunkBin & junk, const IdxTy idx )
{
const bool assert_finite=true;
const Grid & g=m.n.grid();
const IdxTy sz=g.size();
...
else solve_difference_eq_eflux(dy,dx,pr,gp,y,x,m.n,m.p,m.Nteff,e,grfunc,0*c+gp.vt(),flags,junk);
...
update_voltage( state, si, gp,flags,  jb );
break; }
case 4 : { update_carriers_4( state, si, gp,flags,  jb,4 ); break; }
case 5 : { update_carriers_4( state, si, gp,flags,  jb,5 ); break; }
case 6 : { update_carriers_4( state, si, gp,flags,  jb,6 ); break; }
