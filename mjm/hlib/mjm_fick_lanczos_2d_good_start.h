...
/////////////////////////////////////////////////////////////////
// libmesh probably has something better but this is easier to
// modify for now. athough the starter rows are ficked. 
#if 0 
class mesh_indexing 
{
class Tr {
...
// index is the geometric id, x+nx*y+base , with value of the libmesh node id 
typedef MyuIntBlock NodeIndex;
...
const D & _dy): d(distance),nx(_nx),ny(_ny),dx(_dx),dy(_dy) {Init(); } 
...
int relx() const { return int(posx)-int(cnx); } 
int rely() const { return int(posy)-int(cny); } 
int delx() const { return dx*relx(); } 
int dely() const { return dy*rely(); } 
D r2() const { return relx()*relx()*dx*dx+rely()*rely()*dy*dy; } 
IdxTy node() const { return posx+nx*posy; } 

void start() {
// this is signed??? no
intt xx=intt(d/dx+.5);
if (xx>cnx) nminx=0; else nminx=cnx-xx;
nmaxx=cnx+xx;
if (nmaxx>= nx ) nmaxx=nx-1;

intt yy=intt(d/dy+.5);
if (yy>cny) nminy=0; else nminy=cny-yy;
nmaxy=cny+yy;
if (nmaxy>= ny ) nmaxy=ny-1;
posx=nminx;
posy=nmaxy;
...
IdxTy nx,ny,nminx,nminy,nmaxx,nmaxy;
int cnx,cny;
D dx,dy,d;
IdxTy posx,posy;
bool m_ok;
...
mesh_indexing(MyMesh & m, const FlpTy & p, const fl_branches & b) 
	: m_mesh(&m), m_flp(p),m_branches(b) {}
mesh_indexing() : m_mesh(0), m_flp(),m_branches() {}
...
// neighbors
// 
IdxTy north( const IdxTy node ) const { return neighbor(node,0,1); }
IdxTy south( const IdxTy node ) const { return neighbor(node,0,-1); }
IdxTy east( const IdxTy node ) const { return neighbor(node,1,0); }
IdxTy west( const IdxTy node ) const { return neighbor(node,-1,0); }

// this does not quote work on first rows due to element forming 
IdxTy neighbor( const int  node, const int dx, const int dy ) const
{
	int x0=((node%m_n_x));
	int x=(x0+dx);
...
hood_iterator hood_itor(const D & r, const IdxTy node, const D & offx, const D & offy)
{
const int sx=(offx>0)?1:(-1);
const int sy=(offy>0)?1:(-1);
const int cx=(node%m_n_x)+ int(sx*offx/m_dx+.5)*sx;
const int cy=(node/m_n_x)+ int(sy*offy/m_dy+.5)*sy;
round_itor ri(r, m_n_x, m_n_y, m_dx,m_dy);
ri.center( cx,cy);
ri.start();
return ri;

}
node_list within(const D & r, const IdxTy node)
{
node_list nodes;
m_ri=hood_itor(r,node,0,0);
while(m_ri.ok())
{
nodes.push_back(m_ri.node());
m_ri.inc();
}
...
void load_regulars(const FlpTy & flp)
{
	...
	m_dx=(mesh_xmax-mesh_xmin)/m_n_x;
	m_dy=(mesh_ymax-mesh_ymin)/m_n_y;
