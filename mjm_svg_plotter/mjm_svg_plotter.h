...
#ifndef MJM_mjm_svg_plotter_H__
#define MJM_mjm_svg_plotter_H__
 
#include "mjm_globals.h"

#define MJM_RATIONAL_BIGINT mjm_bigint_mmpr
// the svg write uses double doh 
#define ALLOW_IMPLICIT_TO_DOUBLE
...
#include "mjm_svg_plotter_empty.h"
...
/*
made from wizard script on Sat Apr 30 12:56:00 EDT 2022
mjm_svg_plotter
*/
...
class mjm_svg_plotter_params : public mjm_logic_base
{
...
public:
mjm_svg_plotter_params( const StrTy & nm) : Super(nm) {}
mjm_svg_plotter_params() : Super() {}
...
}; // mjm_svg_plotter_params


namespace mjm_svg_plotter_traits
{
class Tr {
public:
...
typedef double D;
// this compiles and runs but the svg fails because
// the "string" for RatTy is of form x/y. Maybe another
// flag to make string an fp? lol 
//typedef RatTy D;
...
}; // mjm_svg_plotter_traits
///////////////////////////////////////////////////////////////




class mjm_mjm_svg_plotter 
{
typedef mjm_svg_plotter_traits::Tr  Tr;
//typedef linc_graph_traits::util  Util;
typedef mjm_mjm_svg_plotter Myt;
protected:
...
typedef mjm_svg_plotter_params ParamGlob;
...
typedef mjm_svg_plotter_base<Tr> DefBase;
typedef DefBase::params_type Pt;
typedef mjm_svg_plotter_empty<Tr> EmptyClass;
typedef mjm_svg_plotter_frame<Tr> FrameClass;
typedef mjm_svg_plotter_scatter_label<Tr> Scatter1Class;
...
typedef mjm_svg_plotter_base<Tr>  BaseClasses;
typedef BaseClasses Piece;
typedef std::map<StrTy,Piece*> PieceMap;
...
public :
mjm_mjm_svg_plotter():m_dmel(new Dmel()) {Init();}
mjm_mjm_svg_plotter(int argc,char **_args) : m_dmel(new Dmel())
{
...
if (s=="-quit") { MM_ERR(" quitt "<<MMPR4(i,argc,args[i],s))  ++i; clean_up(); }
if (s=="-about") { ++i; about(); }
if (confirm) {}
} // cmdlcmd
...
IdxTy make_missing(const StrTy & name, const StrTy & type,const IdxTy flags)
{

auto ii=m_pieces.find(name);
if (ii==m_pieces.end())
{

if (type=="empty") 
{
m_pieces[name]=new EmptyClass(); 
MM_ERR(" makie a  empty ")
}
else if (type=="frame") 
{
m_pieces[name]=new FrameClass(); 
MM_ERR(" makie a  frame  ")
}
else if (type=="scatter") 
{
m_pieces[name]=new Scatter1Class(); 
MM_ERR(" makie a  scatterplot  "<<MMPR(name))
}
else{

MM_ERR(" makie a default ")
 m_pieces[name]=new DefBase(); }
}
...
// make a piece named name from given ragged name 
void cmd_add_piece(Cip & cip , LocalVar & lv ) 
{
const StrTy cmd=cip.cmd();
const StrTy parent=cip.p1;
const StrTy child=cip.p2;
//const StrTy type=(cip.wif(3));
const IdxTy flags=myatoi(cip.wif(3));
MM_ERR(MMPR3(cmd,parent,child)<<MMPR(flags))
//make_missing(name,type,0);
auto ii=m_pieces.find(parent);
auto jj=m_pieces.find(child);
const bool pmissing= (ii==m_pieces.end());
const bool cmissing= (jj==m_pieces.end());
if (pmissing||cmissing)
{
MM_ERR(" not adding as something is missing "<<MMPR2(pmissing,cmissing))
return; 
}
Piece * auck_piece=  m_pieces[parent];
Piece * auck_piece2=  m_pieces[child];

auck_piece->add(auck_piece2,flags);

} //cmd_add_piece


// make a piece named name from given ragged name 
void cmd_make_piece(Cip & cip , LocalVar & lv ) 
{
const StrTy cmd=cip.cmd();
const StrTy name=cip.p1;
const StrTy rag=cip.p2;
const StrTy type=(cip.wif(3));
const IdxTy flags=myatoi(cip.wif(4));
MM_ERR(MMPR4(cmd,name,rag,type)<<MMPR(flags))
make_missing(name,type,0);

Piece * auck_piece=  m_pieces[name];

Ragged & r=m_ragged_map[rag];
MM_ERR(MMPR4(cmd,name,rag,flags)<<MMPR3(rag,type,r.size()))
Pt sht;
auck_piece->ctor(r,type,name,sht,flags);

} //cmd_make_piece

// make a piece named name from given ragged name 
void cmd_write_piece(Cip & cip , LocalVar & lv ) 
{
const StrTy cmd=cip.cmd();
const StrTy fn=cip.p1;
const StrTy name=cip.p2;
...
MM_ERR(MMPR4(cmd,fn,name,flags))
auto ii=m_pieces.find(name);
if (ii==m_pieces.end())
{
MM_ERR(" nothing to write")
return ; 
}

//Piece & piece= * m_pieces[name];
Piece * auck_piece=  m_pieces[name];
if (fn!="-") { std::ofstream os(fn.c_str()); auck_piece->write(os,flags);}
else { auck_piece->write(std::cout,flags);}
...
MM_LOOP(ii,m_pieces) { MM_MSG("m_pieces "<<MMPR2((*ii).first,(*ii).second->size())) }
...
m_cmd_map[StrTy("make-piece")]=&Myt::cmd_make_piece;
m_cmd_map[StrTy("add-piece")]=&Myt::cmd_add_piece;
m_cmd_map[StrTy("write-piece")]=&Myt::cmd_write_piece;
...
{
Ss ss;
ss<<" mjm_mjm_svg_plotter built  "<<__DATE__<<" "<<__TIME__<<CRLF;
ss<<" Mike Marchywka marchywka@hotmail.com started  Sat Apr 30 12:56:00 EDT 2022 "<<CRLF;
...
void Dtor()
{
// kids are not killed in pieces but rather here... 
MM_LOOP(ii,m_pieces) { delete (*ii).second; } 

} 
void Init()
{
MM_ERR("  "<<MMPR2(__FUNCTION__,__FILE__))
m_done=false;
}
