...
mjm_global_credits::credit __credit__mjm_crowd_repulsion("mjm_crowd_repulsion" , "https://ggrepel.slowkow.com/articles/examples.html#hide-some-of-the-labels-1 https://www.aaai.org/Papers/AAAI/2006/AAAI06-167.pdf ");
...
void d_series(const StrTy & s, const IdxTy & f, const IdxTy & n)
{ for(IdxTy i=f; i<(n+f); ++i) m_lo[Series(s,i-f)]=i; m_lo["outsize"]=(n+f); }

IdxTy insize() const { return Kv("insize"); } 
IdxTy outsize() const { return Kv("outsize"); } 
IdxTy textout() const { return Kv("textout"); } 
IdxTy textin() const { return Kv("textin"); } 
IdxTy colorin() const { return Kv("colorin"); } 
IdxTy colorout() const { return Kv("colorout"); } 
...
StrTy Series(const StrTy & s, const IdxTy i )const { Ss ss; ss<<s<<i; return ss.str(); } 
StrTy Dv(const IdxTy i )const { Ss ss; ss<<"g"<<i; return ss.str(); } 
...
if (*p=='g') { d_series("f",i,n); i=i+n; break;}
if (*p=='t') { m_lo["textout"]=i; i=i+1; break;}
if (*p=='c') {   m_lo["colorout"]=i;  i=i+1;  break;}
break;
}// true 
++p;
} // p 
...
{
while (true) { 
// TODO bombs if n is missing....  see kluge .. 
if (*p=='n') {   n=atoi((p+1));   break;}
if (*p=='c') {   m_lo["colorin"]=i;  i=i+1;  break;}
if (*p=='d') { s_left(i,n); i=i+n; break;}
// the "d" is the origin doh.. 
//if (*p=='o') { s_orig(i,n); i=i+n; break;}
if (*p=='t') { m_lo["textin"]=i; i=i+1; break;}
break;
}// true 
// stupid kluge 
MM_ERR(MMPR4(*p,i,j,n))
if (*p=='n') { ++p;}
++p;
} // p 
//m_lo["outsize"]=i;
MM_ERR(MMPR3(m_lo["textin"], m_lo["outsize"],i))
...
void zed_v() { exact.clear(); v=0; MM_LOOP(ii,dv) {(*ii)=0; } ; }
 operator const Loc & () const { return orig;} 
 operator  Loc & () { return orig;} 
...
ss<<MMPR2(v,id);
if (text.length()) ss<<MMPR(text);
if (color.length()) ss<<MMPR(color);
return ss.str();
} 
...
class _normalize_functor
{

typedef std::vector<D> Vals;
public:
_normalize_functor() { Init(); } 
template<class Ty> IdxTy proc( Ty & v,const IdxTy flags)
{
//const bool scale=Bit(flags,0);
//const bool unscale=Bit(flags,1);
if (n==0) minmax(v,0);
return 0;
} // proc
template<class Ty> IdxTy norm( Ty & v,const IdxTy flags)
{
if (n==0) minmax(v,0);
MM_LOOP(ii,v)
{
...
{
if (a[j]==0) p[j]=.5;
else p[j]=a[j]*(p[j]-mins[j]);
} //j 
// the point needs to be aware its was scaled, change orig anc durr... 
} // v 
return 0;
} // norm
template<class Ty,class Tm > IdxTy norm( Ty & v, const Tm & pf, const IdxTy flags)
{
// note this uses the orig points... 
if (n==0) minmax(v,0);
MM_LOOP(ii,v)
{
Vals & p=(&(*ii))->*pf;
...
// this needs to fix the curr AND orig points.. doh 
template<class Ty> IdxTy restore( Ty & v,const IdxTy flags) const 
{
MM_LOOP(ii,v)
{
Vals & p=(*ii);
MM_SZ_LOOP(j,p,psz)
{
MM_ERR(MMPR(__FUNCTION__)<<MMPR4(j,p[j],scale[j],mins[j]))
// if scale is zero, this should be right... 
// need to make a scale ... doh 
if (scale[j]==0)
{
//
// just translate.... 
...
else p[j]=scale[j]*p[j]+mins[j];
} //j 
} // v 
return 0;
} // norm
template<class Ty,class Tm > IdxTy restore( Ty & v,const Tm & pf, const IdxTy flags) const 
{
MM_LOOP(ii,v)
{
Vals & p=(&(*ii))->*pf;
...
template<class Ty> IdxTy minmax( Ty & v,const IdxTy flags)
{
const auto ii=v.begin();
if (ii==v.end()) return ~0;
const Vals & p=(*ii);
if (mins.size()==0)
{
MM_LOOP(jj,p)
...
++n;
const Vals & p=(*ii);
const IdxTy psz=p.size();
if (mins.size()!=psz)
{
MM_ERR("size error "<<MMPR3(n,mins.size(),psz))
}// mins.size
MM_SZ_LOOP(j,p,xpsz)
{
MM_ERR(MMPR(__FUNCTION__)<<MMPR4(j,p[j],mins[j],maxes[j]))
if ( p[j]<mins[j]) mins[j]=p[j];
if ( p[j]>maxes[j]) maxes[j]=p[j];
} //j 

} // v 

MM_SZ_LOOP(j,mins,psz) { scale[j]=maxes[j]-mins[j]; 
// save the value here ... 
if (scale[j]==0){ a[j]=0; }  else a[j]=1.0/scale[j];
 } // j
return 0;
} //proc
...
IdxTy n;
Vals mins,maxes,scale,a;

}; // _normalize_functor

typedef _normalize_functor Normalizer;

class _DisperseParams
{
...
typedef _DisperseParams DisperseParams;
...
{ return disperse(d,s,DisperseParams(),ios,flags); } 
IdxTy disperse(Ragged & d, const Ragged & s, const DisperseParams & dp,  const io_spec_type & ios, const IdxTy flags)
{ return Disperse(d,s,dp,ios,flags); } 
...
ios.sources(lv);
IdxTy ti=ios.textin();
IdxTy ci=ios.colorin();
MM_SZ_LOOP(i,s,ssz)
{
const Line & l=s[i];
point_details pd(i,l,lv );
MM_ERR(MMPR2(l.size(),ti))
if (ti!=(~0U)) pd.text=l[ti];
if (ci!=(~0U)) if (l.size()>ci )  pd.color=l[ci];
pd.zed_v();
m_d.push_back(pd);
...
const bool point_loc=Bit(flags,1);
//bool setmin=false;
...
{ 
 //D &x=pd.curr[j];  
 D &x=point_loc?pd.curr[j]:pd.orig[j];  
 D &xm=maxx[j];  
 D &xmi=minx[j];  
// TODO if all the values are zero then the min is  up 
//if (i==0) { xm=x; xmi=x; if ((x!=0)||real_min){ setmin=true;  xmi=x;}  continue;}
if (i==0) { xm=x; xmi=x;  continue;}
//if (!setmin) {if (x!=0) {setmin=true; xmi=x;continue;  }      } 
if ((x==0)&&!real_min) continue; // for log xform 
if (x>xm) xm=x;
if (x<xmi) if (( x!=0) || real_min) xmi=x;
if (xmi==0) xmi=x;
} // j 
...
D xform(const D & u) { return log(u)/log(10); } 
IdxTy XformPoints( const io_spec_type & ios, const IdxTy flags)
{
const IdxTy fhere=flags>>8;
const bool minrange=Bit(fhere,0); // 1<<8
...
MM_ERR(MMPR(lv.size()))
std::vector<D>  minx(ncoords),maxx(ncoords),range23(ncoords);
for(IdxTy i=0; i<3; ++i) { IdxTy j=0; while (ios.Loc(j,i)!=~0U) { MM_ERR(MMPR3(i,j,ios.Loc(j,i))) ++j; } }
MinMax(minx,maxx,ncoords,0);
for(IdxTy j=0; j<ncoords; ++j) {
MM_ERR(MMPR4(__FUNCTION__,j,minx[j],maxx[j]))
 range23[j]=xform(minx[j])-1.0/3.0*(xform(maxx[j])-xform(minx[j])); } 
...
{ 
 D &x=pd.curr[j];  
MM_ERR(MMPR4(__FUNCTION__,j,x,range23[j]))
 if (x!=0){ x=xform(x); continue; } 
 //else x=log(1e-3); // log(minx[j]/2);
if (minrange) { x=range23[j]; continue; } 
  x= xform(2.0*minx[j]/3.0);
...
{
const bool self_term=Bit(flags,0);
D d=0;
// should use sum of 1/x for worse case thing.. 
const auto & x=p1.curr;
const auto & y=self_term?p2.orig:p2.curr;
...
MM_ERR(MMPR4(i,x[i],y[i],scale[i])<<MMPR(d))
...
ios.dests(lv);
MM_ERR(MMPR(lv.size()))
const IdxTy tcol=ios.textout();
const IdxTy ccol=ios.colorout();
MM_ERR(MMPR2(tcol,ccol))
for(IdxTy i=0; i<3; ++i)
{
IdxTy j=0;
...
//point_details pd(i,l,lv );
//m_d.push_back(pd);
// text, point, grad, potential, theta
for(IdxTy j=0; j<lv.size(); ++j)
{ Ss ss;ss<< pd.curr[j]; l[lv[j]]=ss.str(); } // j 
//MM_ERR(MMPR(l.size()))
// string copy from src orig to dest orig?
for(IdxTy j=0; j<lv.size(); ++j)
{ Ss ss;ss<< pd.orig[j]; l[ios.Loc(j,2)]=ss.str(); } // j 
// this just appends but should have been specificied 
//sae for now... 
for(IdxTy j=0; j<lv.size(); ++j) {Ss ss;  ss<< pd.dv[j]; l.push_back(ss.str()); } 
 {Ss ss;  ss<< pd.v; l.push_back(ss.str()); } 
if (lv.size()>1)
{
const D theta=atan2(pd.dv[0],pd.dv[1]);
 {Ss ss;  ss<< theta;  l.push_back(ss.str()); } 
} // theta
// include text... 
if (tcol!=(~0U))
{
while (l.size()<=tcol) { l.push_back(StrTy());}
 l[tcol]=pd.text;  // Orig
}
if (ccol!=(~0U))
{
while (l.size()<=ccol) { l.push_back(StrTy());}
//  R
StrTy fck=pd.color;
if (fck=="NA") fck="NONE";
 l[ccol]=fck;  // Orig
...
{
auto & pdi=v[i];
// add an attractive potential... 
// this form makes no sense... esp here.. 
//D Vself=Potential(dr,pdi,pdi,scales,1);
//pdi.v+=Vself;
// hopefully the sign is right.. 
//for(IdxTy k=0; k<dr.size(); ++k ) { pdi.dv[k]+=dr[k];  } 
...
if (V==0) { pdi.exact.push_back(j); pdj.exact.push_back(i); }
MM_ERR(MMPR4(i,j,r,V)<<MMPR2(dr[0],dr[1]))
pdi.v+=V; pdj.v+=V;
for(IdxTy k=0; k<dr.size(); ++k ) { pdi.dv[k]+=dr[k]; pdj.dv[k]-=dr[k]; } 

// also want repulsion from points
D Vpt=Potential(dr,pdi,pdj,scales,1);
pdi.v+=Vpt; 
for(IdxTy k=0; k<dr.size(); ++k ) { pdi.dv[k]+=dr[k]; } 
...
IdxTy DispersePoints( const io_spec_type & ios, const DisperseParams &dp, const IdxTy flags)
{
const bool normal_range=Bit(flags,0);
...
else MinMax(minx,maxx,ncoords,1);
// poop
//D expand=.2;
D expand=1;
...
{
D r=maxx[i]-minx[i];
D mini=minx[i]-expand*r;
D maxi=maxx[i]+expand*r;
minl[i]=(mini);
maxl[i]=(maxi); 
...
MM_ERR(MMPR4(i,j,pdi.curr[j],minl[j]))
if ( pdi.curr[j]>(maxl[j])) { pdi.curr[j]=maxl[j]; }
if ( pdi.curr[j]<(minl[j])) { pdi.curr[j]=minl[j]; }
} // j 
...
std::vector<D>   x=pdi.dv;
const std::vector<D> a=attr;
if (ns==1) { } // nothing for now... 
if (ns==2) {
// rare, ocntrivued example of just some points over each other only 
if (x[0]==0) if (x[1]==0)
{
const D vsz=.001;
x[0]=vsz*(maxx[0]-minx[0]); 
x[1]=vsz*(maxx[1]-minx[1]); 
} 
const D stagger=1.0+1.0/(D(olaps+1));
const D theta=2*M_PI*(D(lte)*stagger)/D(olaps+1);
MM_ERR(MMPR4(olaps,lte,theta,stagger))
...
//bool ok=true;
std::vector<D>  temp(pdi.curr.size());
for(IdxTy j=0; j<pdi.curr.size(); ++j) {
//pdi.curr[j]=pdi.curr[j]-vs*pdi.dv[j]; 
MM_ERR(MMPR4(j,pdi.curr[j],pdi.curr[j],vs*fixed[j])<<MMPR(attr[j])) 
temp[j]=pdi.curr[j]-vs*(fixed[j]+0.1*attr[j]); 
//if ( temp[j]>(maxl[j])) { vs=vs*.75; ok=false; break; }
//if ( temp[j]<(minl[j])) { vs=vs*.75; ok=false; break; }
if ( temp[j]>(maxl[j])) { vs=vs*.75; temp[j]=maxl[j]; }
if ( temp[j]<(minl[j])) { vs=vs*.75; temp[j]=minl[j]; }
//pdi.curr[j]=pdi.curr[j]-vs*fixed[j]-0.01*attr[j]; 
} // j  
// doh this does not undo the pathological step    doh 
//for(IdxTy j=0; j<ns; ++j) {
//if ( pdi.curr[j]>(maxl[j])) { vs=vs*.75; ok=false; break; }
//if ( pdi.curr[j]<(minl[j])) { vs=vs*.75; ok=false; break; }
//} // j
for(IdxTy j=0; j<ns; ++j) { pdi.curr[j]=temp[j]; } 
//if (ok) { for(IdxTy j=0; j<ns; ++j) { pdi.curr[j]=temp[j]; } break; } // ok 
} // true
...
IdxTy DispersePointsGrid( const io_spec_type & ios, const DisperseParams &dp, const IdxTy flags)
{
typedef typename point_details::loc_vector Lv;
Lv  lv;
ios.dests(lv);
...
{
IdxTy rc=0;
const bool xform=!Bit(flags,0);
const bool normalize=Bit(flags,1);
const bool denormalize=Bit(flags,2);
IdxTy dflags=0;
if ( normalize)  dflags|=1;
...
if (xform )if (XformPoints(ios,flags)) return 1;
MM_ERR(MMPR(m_d.size()))
// normalize into fixed range,
Normalizer norm;
if ( normalize) {
//norm.norm(m_d,0);
norm.norm(m_d,&point_details::curr,0);
// in good code, this would just do item by item assign buyt
// this is OO crap LOL 
norm.norm(m_d,&point_details::orig,0);
} // normalize

if (!false&&DispersePoints(ios,dp,dflags)) return 1;

if (false) if (DispersePointsGrid(ios,dp,flags)) return 1;

// optionally denorm 
if ( denormalize) {
//norm.restore(m_d,0);
norm.restore(m_d,&point_details::curr,0);
norm.restore(m_d,&point_details::orig,0);
}// denormalize 
...
if (cmd=="quit") break;
if (cmd=="makel") {  // generic data in and out 
...
const IdxTy flags=0+6+0; // turn on norms
MM_ERR(MMPR4(df,sf,flags,cip.wif(3)))
//if (*p=='n') {  i=i+1; n=int(*(p+1));   break;}
//if (*p=='d') { s_left(i,n); i=i+n; break;}
//if (*p=='o') { s_orig(i,n); i=i+n; break;}
//if (*p=='t') { m_lo["textin"]=i; i=i+1; break;}
//ios.s_left("n2dt");
...
if (cmd=="make") {  // generic data in and out 
Myt::io_spec_type ios;
//loc_vector in,out; IdxTy algo, win,wout;
const StrTy df=cip.p1;
const StrTy sf=cip.p2;
const IdxTy flags=1+6; // turn on norms
MM_ERR(MMPR4(df,sf,flags,cip.wif(3)))
//if (*p=='n') {  i=i+1; n=int(*(p+1));   break;}
//if (*p=='d') { s_left(i,n); i=i+n; break;}
//if (*p=='o') { s_orig(i,n); i=i+n; break;}
