...
{ return imap_copy_messages_mjm(m,el,dest,delete_original,0); } 
int imap_copy_messages_mjm(struct Mailbox *m, struct EmailList *el, const char *dest, bool delete_original, int mjm_flags)
{
bool headless=((mjm_flags&1)!=0);
bool create_missing=((mjm_flags&2)!=0);
printf("  iun imap_copy_messages %d %d %s\n",m,el,dest);
  if (!m || !el || !dest)
{
    return -1;
}
  ...
  do
  {
	...
	printf(" fs  imap_sync %d %s\n ",rc,dest);
      ...
      if (rc == 0)
      {
        ...
        rc = -1;
	printf(" fs imap_sync no messages  %d %s\n ",rc,dest);
        goto out;
      }
      else if (rc < 0)
      {
        mutt_debug(LL_DEBUG1, "#1 could not queue copy\n");
	printf(" fs imap_sync bad que  %d %s\n ",rc,dest);
        goto out;
      }
      else
      {
	...
	printf(" fs imap_exec again  bad   %d %s\n ",rc,dest);
    if (rc == IMAP_EXEC_ERROR)
    {
      ...
      printf(" making a prompt from message.c for move  %s\n", mbox);
      snprintf(prompt, sizeof(prompt), _("Create %s?"), mbox);
if (!headless)
{
      if (C_Confirmcreate && (mutt_yesorno(prompt, MUTT_YES) != MUTT_YES))
      {
      printf(" done a prompt from message.c for move  %s\n", mbox);
        mutt_clear_error();
        goto out;
      }
...
else
{
	if (!create_missing)
      {
      printf(" done since no create_missing  move  %s\n", mbox);
        mutt_clear_error();
        goto out;
      }

} // headless
      printf(" try to create   %s", mbox);
  ...
  if (rc != 0)
  {
printf(" fs imap_execerror  again  bad    %d %s\n ",rc,dest);
