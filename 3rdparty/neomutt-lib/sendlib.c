...
{
printf("int mutt_write_mime_body enter %s \n",a->filename);
//if (a->filename == 0 ) return 0; 
  ...
  if (a->type == TYPE_MULTIPART)
  {
    ...
    if (!p)
    {
	printf(" no boundary paramter adcasd\n");
    ...
    {
	printf("writing part header \n");
      fprintf(fp, "\n--%s\n", boundary);
      if (mutt_write_mime_header(t, fp) == -1)
        return -1;
	printf("writing part bodyr \n");
      fputc('\n', fp);
      if (mutt_write_mime_body(t, fp) == -1)
        return -1;
    }
	printf("writing part boundary  \n");
    fprintf(fp, "\n--%s--\n", boundary);
    return ferror(fp) ? -1 : 0;
  }
      ...
      (mutt_str_strcmp(a->subtype, "pgp-encrypted") == 0) && !a->filename)
  {
	...
	printf("writing more crap %s    \n", a->filename);
//if (a->filename==0) return 0;
  fp_in = fopen(a->filename, "r");
  if (!fp_in)
  {
	printf("error with fp_in  %s  \n",a->filename);
...
printf(" mutt_update_encoding f  %s \n",a->filename);
  set_encoding(a, info);
  mutt_stamp_attachment(a);
printf(" mutt_update_encoding %s \n",a->filename);
  FREE(&a->content);
  a->content = info;
}
...
printf(" trying to attach in mutt_make_file_attach %s   %s\n",att->filename , path);
  ...
  if (!info)
  {
	printf(" could not attach something...\n");
    mutt_body_free(&att);
    return NULL;
  }
  ...
  if (!att->subtype)
  {
        ...
        ((info->lobin == 0) || ((info->lobin + info->hibin + info->ascii) / info->lobin >= 10)))
    {
      ...
      att->type = TYPE_TEXT;
	printf(" sneding as text\n");
      att->subtype = mutt_str_strdup("plain");
    }
    else
    {
	printf(" sneding as non-text\n");
      att->type = TYPE_APPLICATION;
      att->subtype = mutt_str_strdup("octet-stream");
    }
  }
else printf(" subtype wtf %s   %s   %s\n",att->subtype, att->filename , path);
