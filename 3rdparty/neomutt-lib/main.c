...
#define MJM_NEOMUTT_LIB
//#define MJM_NEOMUTTDBG
...
* moved the crap out of way, put into a reusable file for now 
 */
//static void init_locale(void)
 void init_locale(void)
{
...
int main_exit_shutdown(struct cmd_line_stuff * pcls, struct MjmMsgLists* pmml)
{
  MuttLogger = log_disp_queue;
  mutt_buffer_dealloc(&(*pcls).folder);
  mutt_buffer_dealloc(&(*pcls).expanded_infile);
  mutt_buffer_dealloc(&(*pcls).tempfile);
  mutt_list_free(&(*pmml).queries);
  crypto_module_free();
...
int main_curses_shutdown(struct cmd_line_stuff * pcls, struct MjmMsgLists* pmml)
{

  mutt_endwin();
  mutt_unlink_temp_attachments();
  /* Repeat the last message to the user */
  if ((*pcls).repeat_error && ErrorBufMessage)
    puts(ErrorBuf);
return main_exit_shutdown(pcls,pmml);

}
int main_ok_shutdown(struct cmd_line_stuff * pcls, struct MjmMsgLists* pmml)
{
(*pcls).rc=0;
return main_curses_shutdown(pcls,pmml);
}
...
int shutdown_loop(struct cmd_line_stuff * pcls, struct MjmMsgLists* pmml)
{ //return shutdown_loop(&cls,&mml); //   cls.rc = 0;
      ctx_free(&Context);
      log_queue_empty();
      (*pcls).repeat_error = false;
shutdown_login();
return main_ok_shutdown(pcls,pmml); 
}

struct main_send_flags msf;
struct MjmMsgLists mml;
struct main_flags mf;
struct cmd_line_stuff cls;

 int  shutdown_mutt() //   cls.rc = 0;
{  return shutdown_loop(&cls,&mml); }  //   cls.rc = 0;

#ifdef MJM_NEOMUTT_LIB
extern int main_lib(int argc, char *argv[], char *envp[]);
#else
 ...
 */
#ifdef MJM_NEOMUTT_LIB
int main_lib(int argc, char *argv[], char *envp[])
#else
int main(int argc, char *argv[], char *envp[])
#endif
{
zero_my_hero(&msf,sizeof(msf));

#ifdef MJM_NEOMUTTDBG
printf(" enter key \n");
int x;
scanf("%d",&x);
#endif
...
*/
cmd_line_stuff_init(&cls,argc);

  MuttLogger = log_disp_terminal;
if ( 0!= formalities( argc, argv, envp) ) { goto  main_exit; } 
////////////////////////////////////////////////////////////////
#if 1 

 for (optind = 1; optind < cls.double_dash;)
  {
    ...
    for (; optind < argc; optind++)
    {
      if ((argv[optind][0] == '-') && (argv[optind][1] != '\0'))
      {
        if ((argv[optind][1] == '-') && (argv[optind][2] == '\0'))
          cls.double_dash = optind; /* quit outer loop after getopt */
        break;                  /* drop through to getopt */
      }

      /* non-option, either an attachment or address */
      if (!STAILQ_EMPTY(&mml.attach))
        mutt_list_insert_tail(&mml.attach, mutt_str_strdup(argv[optind]));
      else
        argv[cls.nargc++] = argv[optind];
    }

    /* USE_NNTP 'g:G' */
    //i = getopt(argc, argv, "+A:a:Bb:F:f:c:Dd:l:Ee:g:GH:i:hm:npQ:RSs:TvxyzZ");
    i = getopt(argc, argv, "+A:a:Bb:F:f:c:Dd:l:Ee:g:GH:i:hm:MnpQ:RSs:TvxyzZ");
    if (i != EOF)
    {
      switch (i)
      {
        case 'A':
          mutt_list_insert_tail(&mml.alias_queries, mutt_str_strdup(optarg));
          break;
        case 'a':
          mutt_list_insert_tail(&mml.attach, mutt_str_strdup(optarg));
          break;
        case 'B':
          mf.batch_mode = true;
          break;
        case 'M':
          mjm_daemon_mode = true;
          OptNoCurses = true;
          break;
        case 'b':
          mutt_list_insert_tail(&mml.bcc_list, mutt_str_strdup(optarg));
          break;
        case 'c':
          mutt_list_insert_tail(&mml.cc_list, mutt_str_strdup(optarg));
          break;
        case 'D':
          mf.dump_variables = true;
          break;
        case 'd':
          msf.dlevel = optarg;
          break;
        case 'E':
          ...
          break;
        case 'e':
          mutt_list_insert_tail(&mml.commands, mutt_str_strdup(optarg));
          break;
        case 'F':
          mutt_list_insert_tail(&Muttrc, mutt_str_strdup(optarg));
          break;
        case 'f':
          mutt_buffer_strcpy(&cls.folder, optarg);
          mf.explicit_folder = true;
          break;
#ifdef USE_NNTP
...
#endif
        case 'H':
          msf.draft_file = optarg;
          break;
        case 'i':
          msf.include_file = optarg;
          break;
        case 'l':
          msf.dfile = optarg;
          break;
        ...
        case 'p':
          ...
          break;
        case 'Q':
          mutt_list_insert_tail(&mml.queries, mutt_str_strdup(optarg));
          break;
        case 'R':
          flags |= MUTT_CLI_RO; /* read-only mode */
          break;
        case 'S':
          mf.hide_sensitive = true;
          break;
        case 's':
          msf.subject = optarg;
          break;
...
//////////////////////////////////////////////////////////////////////////
  ...
  if (msf.dfile)
  {
    cs_str_initial_set(cls.cs, "debug_file", msf.dfile, NULL);
    cs_str_reset(cls.cs, "debug_file", NULL);
  }

  if (msf.dlevel)
  {
    short num = 0;
    if ((mutt_str_atos(msf.dlevel, &num) < 0) || (num < LL_MESSAGE) || (num >= LL_MAX))
    {
      mutt_error(_("Error: value '%s' is invalid for -d"), msf.dlevel);
      goto main_exit; // TEST07: neomutt -d xyz
    }
    cs_str_initial_set(cls.cs, "debug_level", msf.dlevel, NULL);
    cs_str_reset(cls.cs, "debug_level", NULL);
  }
  ...
  MuttLogger = log_disp_queue;
// this makes a copy of mml but it is ok... 
get_cc_lists(&e,mml);
  /* Check for a batch send. */
  if ((!isatty(0)&&!mjm_daemon_mode) || !STAILQ_EMPTY(&mml.queries) || !STAILQ_EMPTY(&mml.alias_queries) ||
      mf.dump_variables || mf.batch_mode)
  {
    OptNoCurses = true;
  ...
  int rc2 = mutt_init(cls.cs, flags & MUTT_CLI_NOSYSRC, &mml.commands);
  mutt_list_free(&mml.commands);
  if (rc2 != 0)
    goto main_curses;
  ...
  if (msf.cli_nntp)
    cs_str_string_set(cls.cs, "news_server", msf.cli_nntp, NULL);
  if (!C_NewsServer)
  {
    const char *env_nntp = mutt_str_getenv("NNTPSERVER");
    cs_str_string_set(cls.cs, "news_server", env_nntp, NULL);
  }
  if (!C_NewsServer)
  {
    char buf[1024];
    char *server = mutt_file_read_keyword(SYSCONFDIR "/nntpserver", buf, sizeof(buf));
    cs_str_string_set(cls.cs, "news_server", server, NULL);
  }
  if (C_NewsServer)
    cs_str_initial_set(cls.cs, "news_server", C_NewsServer, NULL);
#endif
  ...
  if (msf.new_type)
  {
    ...
    int r = cs_str_initial_set(cls.cs, "mbox_type", msf.new_type, &err);
    ...
    cs_str_reset(cls.cs, "mbox_type", NULL);
  }

  if (!STAILQ_EMPTY(&mml.queries))
  {
    cls.rc = mutt_query_variables(&mml.queries);
    goto main_curses;
  }

  if (mf.dump_variables)
  {
    dump_config(cls.cs, mf.hide_sensitive ? CS_DUMP_HIDE_SENSITIVE : CS_DUMP_NO_FLAGS, stdout);
    goto main_ok; // TEST18: neomutt -D
  }

  if (!STAILQ_EMPTY(&mml.alias_queries))
  {
    cls.rc = 0;
    for (; optind < argc; optind++)
      mutt_list_insert_tail(&mml.alias_queries, mutt_str_strdup(argv[optind]));
    struct ListNode *np = NULL;
    STAILQ_FOREACH(np, &mml.alias_queries, entries)
    {
    ...
    mutt_list_free(&mml.alias_queries);
  ...
  if (!mjm_daemon_mode) 
{
  ...
  if (mjm_daemon_mode) 
{
notify_observer_add(NeoMutt->notify, mjm_hist_observer, NULL);
  notify_observer_add(NeoMutt->notify, mjm_log_observer, NULL);
  notify_observer_add(NeoMutt->notify, mjm_menu_config_observer, NULL);
  notify_observer_add(NeoMutt->notify, mjm_reply_observer, NULL);
  notify_observer_add(NeoMutt->notify, mjm_abort_key_config_observer, NULL);
...
if (Colors)
  ...
  if (sendflags & SEND_POSTPONED)
  {
    ...
    log_queue_empty();
    cls.repeat_error = true;
    goto main_curses;
  }
  else if (msf.subject || e || sendflags || msf.draft_file || msf.include_file ||
           !STAILQ_EMPTY(&mml.attach) || (optind < argc))
  {
struct compose_files cf;
zero_my_hero(&cf,sizeof(cf));
    ...
    {
	if (0!=add_mailto(e,&cf.bodytext,argv[i]))
	{
    ...
    if (!msf.draft_file && C_Autoedit && TAILQ_EMPTY(&e->env->to) &&
    ...
    if (msf.subject)
      e->env->subject = mutt_str_strdup(msf.subject);

    if (msf.draft_file)
    {
      cf.infile = msf.draft_file;
      msf.include_file = NULL;
    }
    else if (msf.include_file)
      cf.infile = msf.include_file;
    else
      mf.edit_infile = false;

    if (cf.infile || cf.bodytext)
    {
	...
	if (0!=draft_proc(e,cf,msf,cls ,C_ResumeEditedDraftFiles ))
{
          email_free(&e);
          goto main_curses; // TEST
}
      ...
      else if (mf.edit_infile)
        cf.bodyfile = mutt_b2s(&cls.expanded_infile);
      // For bodytext and unedited include_file: use the tempfile.
      else
        cf.bodyfile = mutt_b2s(&cls.tempfile);
...
if (0!=attach_stuff(e,&mml)) 
{
   ...
   //  mutt_list_free(&mml.attach);
    ...
    cf.rv = mutt_send_message(sendflags, e, cf.bodyfile, NULL, NULL);
    ...
    if (mf.edit_infile)
    {
      ...
      else if (msf.draft_file)
      {
        if (truncate(mutt_b2s(&cls.expanded_infile), 0) == -1)
        {
          mutt_perror(mutt_b2s(&cls.expanded_infile));
          email_free(&e);
          goto main_curses; // TEST33: neomutt -H read-only -s test john@example.com -E
        }
        cf.fp_out = mutt_file_fopen(mutt_b2s(&cls.expanded_infile), "a");
        if (!cf.fp_out)
        {
          mutt_perror(mutt_b2s(&cls.expanded_infile));
          email_free(&e);
        ...
        mutt_rfc822_write_header(
            cf.fp_out, e->env, e->content, MUTT_WRITE_HEADER_POSTPONE, false,
            C_CryptProtectedHeadersRead && mutt_should_hide_protected_subject(e));
        if (C_ResumeEditedDraftFiles)
          fprintf(cf.fp_out, "X-Mutt-Resume-Draft: 1\n");
        fputc('\n', cf.fp_out);
        if ((mutt_write_mime_body(e->content, cf.fp_out) == -1))
        {
          mutt_file_fclose(&cf.fp_out);
          email_free(&e);
    ...
    if (!mutt_buffer_is_empty(&cls.tempfile))
      unlink(mutt_b2s(&cls.tempfile));
  ...
  else
  {
    if (flags & MUTT_CLI_MAILBOX)
    {
      ...
      mutt_buffer_reset(&cls.folder);
      mutt_mailbox_next_buffer(Context ? Context->mailbox : NULL, &cls.folder);
#ifdef USE_IMAP
    ...
    else if (flags & MUTT_CLI_SELECT)
    {
      ...
      mutt_buffer_reset(&cls.folder);
      mutt_buffer_select_file(&cls.folder, MUTT_SEL_FOLDER | MUTT_SEL_MAILBOX, NULL, NULL);
      if (mutt_buffer_is_empty(&cls.folder))
      {
    ...
    if (mutt_buffer_is_empty(&cls.folder))
    {
      if (C_Spoolfile)
      {
        ...
        if (m_desc)
          mutt_buffer_strcpy(&cls.folder, m_desc->realpath);
        else
          mutt_buffer_strcpy(&cls.folder, C_Spoolfile);
      }
      else if (C_Folder)
        mutt_buffer_strcpy(&cls.folder, C_Folder);
      /* else no folder */
    }
    ...
    {
      OptNews = false;
      mutt_buffer_alloc(&cls.folder, PATH_MAX);
      nntp_expand_path(cls.folder.data, cls.folder.dsize, &CurrentNewsSrv->conn->account);
    }
    else
#endif
      mutt_buffer_expand_path(&cls.folder);

    mutt_str_replace(&CurrentFolder, mutt_b2s(&cls.folder));
    mutt_str_replace(&LastFolder, mutt_b2s(&cls.folder));

    if (flags & MUTT_CLI_IGNORE)
    {
      /* check to see if there are any messages in the folder */
      switch (mx_check_empty(mutt_b2s(&cls.folder)))
      {
        case -1:
          mutt_perror(mutt_b2s(&cls.folder));
    ...
    if (!Context)
    {
      ...
      mailbox_free(&m);
      mutt_error(_("Unable to open mailbox %s"), mutt_b2s(&cls.folder));
      cls.repeat_error = false;
    }
    if (Context || !mf.explicit_folder)
    {
#ifdef USE_SIDEBAR
...
{
printf(" should output stuff here\n");
      struct MuttWindow *dlg = index_pager_init();
...
main_ok:
 return main_ok_shutdown(&cls,&mml); //   cls.rc = 0;
main_curses:
 return main_curses_shutdown(&cls,&mml); //   cls.rc = 0;
#if 0
  mutt_endwin();
  mutt_unlink_temp_attachments();
  /* Repeat the last message to the user */
  if (cls.repeat_error && ErrorBufMessage)
    puts(ErrorBuf);
#endif

main_exit:
return main_exit_shutdown(&cls,&mml);
#if 0
int main_exit_shutdown(struct cmd_line_stuff * pcls)
{
  MuttLogger = log_disp_queue;
  mutt_buffer_dealloc(&(*pcls).folder);
  mutt_buffer_dealloc(&(*pcls).expanded_infile);
  mutt_buffer_dealloc(&(*pcls).tempfile);
  mutt_list_free(&mml.queries);
  crypto_module_free();
  mutt_window_free_all();
  mutt_buffer_pool_free();
  mutt_envlist_free();
