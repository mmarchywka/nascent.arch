...
static int generate_body_auto(FILE *fp_tmp, struct Email *e, SendFlags flags,
                         struct Mailbox *m, struct EmailList *el, bool inc)
{
  struct Body *tmp = NULL;
  struct EmailNode *en = NULL;
  bool single = true;

  if (el) en = STAILQ_FIRST(el);
  if (en) single = !STAILQ_NEXT(en, entries);

  /* An EmailList is required for replying and forwarding */
  if (!el && (flags & (SEND_REPLY | SEND_FORWARD))) return -1;

  if (flags & SEND_REPLY)
  {
    enum QuadOption ans = inc?MUTT_YES:MUTT_NO;
//        query_quadoption(C_Include, _("Include message in reply?"));
//    if (ans == MUTT_ABORT) return -1;
    if (ans == MUTT_YES)
    {
   printf("Including quoted message...\n");
      if (single && en)
        include_reply(m, en->email, fp_tmp);
      else
      {
   printf("Including quoted messagessssssssssssssssss...\n");
        STAILQ_FOREACH(en, el, entries)
        {
          if (include_reply(m, en->email, fp_tmp) == -1)
          {
...
static int invoke_mta(struct Email *e)
{
  ...
  if (!fp_tmp)
{
	printf(" could not make temp %s\n",mutt_b2s(tempfile));
    goto cleanup;
}
	printf(" could  make temp %s\n",mutt_b2s(tempfile));
#ifdef USE_SMTP
  ...
  fputc('\n', fp_tmp); /* tie off the header. */
	...
	printf(" adscasdcs   %d  %s\n",ferror(fp_tmp),mutt_b2s(tempfile));
  if ((mutt_write_mime_body(e->content, fp_tmp) == -1))
{
	printf(" could not write mime bod  %s\n",mutt_b2s(tempfile));
    goto cleanup;
}
                      ...
                      struct Context *ctx, struct EmailList *el)
{
      ...
      !((flags & SEND_DRAFT_FILE) && C_ResumeDraftFiles))
  {
    ...
    if ((flags & SEND_BATCH)&&(!(flags&SEND_MJM_ASIS)))
    {
  ...
  mjm_daemon_mode?(generate_body_auto(fp_tmp, e_templ, flags, ctx ? ctx->mailbox : NULL, el,true) == -1):
