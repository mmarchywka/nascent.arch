...
{
//printf(" FUCK xxx loc a\n");
//char cb[1<<24];
//char cb[1<<16];
//MM_ERR("test")
char * cb= new char[1<<24];
//printf(" FUCK xxx loc b\n");
	...
	int dn=mjm_format(px,p,fsp);
      ...
      if (fsp->print_stacked)  sprintf(cb,"%s %d %s %s",pfx.c_str(), lvl,cur_node->name,px);  
	printf("%s\n",cb);
...
if (fsp->print_stacked) sprintf(cb,"%s %d %s",pfx.c_str(),lvl,c->name);
printf("%s\n",cb);
	    ...
	    xmlElementContentPtr content)
{
    fprintf(stdout, "SAX.elementDecl(%s, %d, ...)\n",
            name, type);
// fuck this is toin to take foruck forever for this stht fuck 
...
fprintf(stderr," fsp : stack %d print_stacked %d print_curr %d remove_Crlf %d\n",fsp->stack, 
...
//quiet_mjm_xml();
	...
	{ ++i; if (i<argc)  mjm_flags=atoi(argv[i]); } 
