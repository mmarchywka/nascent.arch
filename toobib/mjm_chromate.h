...
// g++  -Wall -std=gnu++11 -DTEST_MJM_CHROMATE -I. -I../../mjm/hlib -I../../mjm/num  -I../../pkg/include -Wno-unused-variable -Wno-misleading-indentation  -gdwarf-3 -O0  -x c++ mjm_chromate.h  -lpthread -lreadline -o chromate.out
...
class FetchInfo {
public:
FetchInfo(): rc(~0),eline(0),right_one(false) {}
...
bool AppearsAlive() const { return Bit(m_state,0); } 
...
{
MM_ERR(" testing line "<<MMPR(i))
Ragged::Line & l=r[i];
const IdxTy len=l.size();
if (len<3) continue;
Ss ss;
MM_ERR(" looking at  line "<<MMPR(l[len-1]))
//MM_LOOP(ii,l) ss<<(*ii)<<"|";
...
{
/*
StrTy spfx="{\"sessionId\":\""+sess+"\",\"method\":\"";
//StrTy ua="foo doo" ; // "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36";
StrTy ua= "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36";
// A similar command is in Network.setUserAgentOverride
StrTy uacmd=spfx+"Emulation.setUserAgentOverride\",\"params\":{\"userAgent\":\""+ ua + "\"},\"id\":";
r=CmdGet(fi,IdAdd(uacmd),flags);
StrTy plat="__proto__";
...
// {"method":"Target.createTarget","params":{"url":""},"id":4}
if (m_target=="") { 
...
fetch_info fi;
StrTy cmds1= "{\"method\":\"Target.createTarget\",\"params\":{\"url\":\"\"},\"id\":"; 
StrTy cmd=IdAdd(cmds1); // ss.str();
Ragged r=CmdGet(fi,cmd,flags);
...
//StrTy cmd= "{\"method\":\"Target.setDiscoverTargets\",\"params\":{\"discover\":true},\"id\": 1}" ;
StrTy cmds1= "{\"method\":\"Target.setDiscoverTargets\",\"params\":{\"discover\":true},\"id\":"; //  1}" ;
//StrTy cmds2="}"; Ss ss; ss<<cmds1; ss<<m_id_cnt; ss<<cmds2;MM_ERR(MMPR(ss.str()))
StrTy cmd=IdAdd(cmds1); // ss.str();
...
if (m_session=="")
{
//> {"method":"Target.attachToTarget","params":{"targetId":"CE134BF9DAF2DC1AFA4D51706BBE8E07","flatten":true},"id": 2}
StrTy tic="{\"method\":\"Target.attachToTarget\",\"params\":{\"targetId\":\""+
m_target+"\",\"flatten\":true},\"id\":"; // 
StrTy cmd=IdAdd(tic);
Ragged r=CmdGet(fi,cmd,flags);
StrTy sess=m_json.any_kv(r,"sessionId",0,flags);
m_session=sess;
MM_ERR(MMPR(sess))
StrTy spfx= SPFX(); // "{\"sessionId\":\""+sess+"\",\"method\":\"";
//StrTy ua="foo doo" ; // "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36";
...
StrTy plat="__proto__";
StrTy nocmd=spfx+"Emulation.setNavigatorOverrides\",\"params\":{\"platform\":\""+ plat + "\"},\"id\":";

StrTy expr="delete window.navigator.webdriver";
StrTy nocmd2=spfx+"Runtime.evaluate\",\"params\":{\"expression\":\""+ expr + "\"},\"id\":";
// cmd={"sessionId":"4C09490E8D5A1FEE597576C6B0CB91CC","method":"Runtime.evaluate","params":{"expression":"delete window.navigator.webdriver"},"id":99}
// setting blank seems to work but complains js is not enabled 
...
if ((rc!=0)||fi.error.length()) throw fi.line(__LINE__);
...
fetch_info PrintPageOld(const StrTy & fn, const StrTy & url, const IdxTy flags) 
{
fetch_info fi;
IdxTy rc=GoToPage(fi,url,flags);
if ((rc!=0)||fi.error.length())
{
MM_ERR(" aborting pageprint due to  due to problm.. "<<MMPR3(fn,url, fi.error))
fi.doc="";
return fi;
}
// just the core let the thing finish it
...
StrTy nacmd=spfx+"Page.navigate\",\"params\":{\"url\":\""+url+"\"},\"id\":";
Ragged r=CmdGet(fi,IdAdd(nacmd),flags);
if (fi.error!="" ) return 1; 
// is this order wrong??????
//StrTy jscmd=spfx+"Emulation.setScriptExecutionDisabled\",\"params\":{\"value\":"+StrTy("false")+"},\"id\":";
//r=CmdGet(fi,IdAdd(jscmd),flags);
//if (fi.error!="" ) return 1; 
...
//StrTy gdcmd=spfx+"DOM.getDocument\",\"params\":{\"url\":\""+url+"\"},\"id\":";
//StrTy gdcmd=spfx+"DOM.getDocument\",\"params\":{\"depth\":-1,\"pierce\":true},\"id\":";
StrTy gdcmd=spfx+"DOM.getDocument\",\"id\":";
...
#if 0 
fetch_info DownloadOld(const StrTy & fn, const StrTy & url, const IdxTy flags) 
{ 
...
expr+="  const a = document.createElement(\\\"a\\\"); a.style.display = 'none'; a.href = url; a.download = \'"+fn+"\'; document.body.appendChild(a); a.click(); window.URL.revokeObjectURL(url);";
...
StrTy nocmd="";
StrTy nocmd2=spfx+"Runtime.evaluate\",\"params\":{\"expression\":\""+ expr + "\"},\"id\":";
//StrTy nocmd2=spfx+"Network.loadNetworkResource\",\"params\":{\"url\":\""+ url + "\",\"options\":{\"disableCache\":true,\"includeCredentials\":false}},\"id\":";
// cmd={"sessionId":"4C09490E8D5A1FEE597576C6B0CB91CC","method":"Runtime.evaluate","params":{"expression":"delete window.navigator.webdriver"},"id":99}
// setting blank seems to work but complains js is not enabled 
...
//StrTy nocmd3=spfx+"Page.setDownloadBehavior\",\"params\":{\"behavior\":\""+ "allow" + "\",\"downloadPath\":\"./chromate_downloads\"},\"id\":";


if (false) { 
StrTy nocmd3=spfx+"Page.setDownloadBehavior\",\"params\":{\"behavior\":\""+ "allow" + "\",\"downloadPath\":\""+m_download_path+"\"},\"id\":";
Ragged rs;
...
{
MM_ERR("saving to zzzz"<<MMPR(fi.doc.length()))
