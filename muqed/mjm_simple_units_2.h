...
MM_ERR(MMPR3(ct.dump(),rt.dump(),rf.dump()))
MM_LOOP(ii,vc)
{
const conv_type & x=(*ii);
MM_ERR(MMPR3(level,ct.dump(),x.dump()))
//if (x.can_mult(ct)) {
...
// ffttv
conversion_type ftfock(x.to_unit(),x.to_thing(),ct.to_unit(),ct.to_thing(), ct.v()*x.v());
const IdxTy tc=Convert(r,ftfock,vc2,flags,level+1);
if (tc==0) return 0;
}
const conv_type y= x.flip();
MM_ERR(MMPR3(level,ct.dump(),y.dump()))
//if (y.can_mult(ct)) {
if (y.can_mult(rf)) {
if (y.same_units(ct))
{
...
Vc vc2; MM_LOOP(jj,vc) { if (ii!=jj) vc2.push_back(*jj); } 
conversion_type ftfock(y.to_unit(),y.to_thing(),ct.to_unit(),ct.to_thing(), ct.v()*y.v());
const IdxTy tc=Convert(r,ftfock,vc2,flags,level+1);
if (tc==0) return 0;
}
