...
class _new_qty_typed
{
typedef _new_qty_typed Myt;
typedef IdxTy NounCoTy;
static IdxTy bad(const IdxTy x) { return ~0; } 
static StrTy bad(const StrTy &  x) { return StrTy(); } 
public:
typedef  typename Qdim::key_type t_u_type;
_new_qty_typed() : m_empty(true),m_error(false) {}
_new_qty_typed(const D & v) :m_noun(bad(NounCoTy())), m_val(v),m_empty(false),m_error(false) {Init();}
// these changed meaning 
// a string is either noun or units, Qdim ctor can take a top and bottom
// "of" to append to units. 
//_qty_typed(const D & v, const IdxTy t) : m_v(v),m_type(t),m_empty(false),m_error(false)  {}
//_qty_typed(const D & v, const StrTy& sfx) : m_v(v),m_type(~0),m_val(v,sfx),m_empty(false),m_error(false)  {}
_new_qty_typed(const D & v, const StrTy& sfx)  
 :m_noun(bad(NounCoTy())), m_val(v,sfx),m_empty(false),m_error(false)  {Init();}
_new_qty_typed(const NounCoTy & nm,const D & v, const StrTy& sfx)  
 :m_noun(nm), m_val(v,sfx),m_empty(false),m_error(false)  {Init();}
_new_qty_typed(const Qdim& qd) :m_noun(bad(NounCoTy())), m_val(qd),m_empty(false),m_error(false)  {Init();}
_new_qty_typed(const NounCoTy & nm,const Qdim& qd) 
	:m_noun(nm), m_val(qd),m_empty(false),m_error(false)  {Init();}
_new_qty_typed(const NounCoTy & nm) 
	:m_noun(nm), m_val(0),m_empty(false),m_error(false)  {Init(); }

void restring(St & st ) {Init(st); } 
 operator StrTy () const { return m_string; } 
const D & qty() const { return m_val.qty();}
void add_raw(const D & x )  {  m_val.add_raw(x);}
void set_qty(const D & x )  {  m_val.set(x);}
const StrTy sfx() const { return m_val.sfx();}
t_u_type t_u_sfx() const   { return m_val.t_u_sfx(); } 
const NounCoTy &  noun() const { return m_noun;}
bool valid() const { return !m_error; } 
bool empty() const { return m_empty; } 
void  invalidate()  { m_error=true; } 
void add(const D & del) { m_val.add_raw(del); }
Myt operator+(const Myt & x)
{
Myt y=(*this);
y+=x;
...
const bool nouns_ok=(m_noun==x.m_noun)||(x.m_noun==bad());
const bool sfxs_ok=(sfx()==x.sfx())||(x.m_noun==bad());
 if( (!nouns_ok)||(!sfxs_ok)) { 
if (x.qty()!=0)
{ MM_ERR(" types don't match "<<MMPR2(x.m_noun,m_noun)<<MMPR3(nouns_ok,x.m_val.dump(),m_val.dump()))}
m_error=true;
return (*this); 
} 
m_val.add_raw(x.qty());
...
{
if (m_noun<x.m_noun) return true;
if (m_noun>x.m_noun) return !true;
return m_val.lexi_lt(x.m_val);
}
private:
void Init() 
{
...
typedef _new_qty_typed qty_typed;

class _indexed_typeds
{
typedef _indexed_typeds Myt;
typedef qty_typed Tgt;

typedef std::vector<Tgt> Vec;
typedef std::map<IdxTy,IdxTy> Map;
typedef std::map<IdxTy,IdxTy> IdxMap;
typedef std::map<StrTy,IdxTy> StrMap;

typedef typename Map::iterator BaseItor;
#if 0 
class _iterator 
{
typedef _iterator Myt;
typedef Map M;
typedef typename M::key_type base_key_type;
typedef typename M::mapped_type base_mapped_type;
typedef typename M::value_type base_value_type;
typedef Tgt mapped_type; 
//typedef std::pair<base_key_type,mapped_type> value_type;
typedef std::pair<IdxTy,IdxTy> value_type;
public:

_iterator( ) 
:m_bi(),m_pmap(0), m_pvec(0) {}

_iterator(BaseItor & bi, Map & m, Vec & v ) 
:m_bi(bi),m_pmap(&m), m_pvec(&v) {}

//value_type operator*() const { return value_type(m_loc,(*v)[(*m_pmap)[m_loc]]); } 
//value_type operator*() const { return value_type(m_bi.first,(*v)[(*m_pmap)[(*m_bi).second]]); } 
//value_type operator*() const { return value_type(m_bi.first,(*v)[(*m_pmap)[(*m_bi).second]]); } 
Myt & operator++() { ++m_bi; return *this; }
bool operator==(const Myt & x) { return m_bi==x.m_bi; }
private:
BaseItor m_bi;
//IdxTy m_loc;
Map * m_pmap;
...
public:
typedef Tgt target_type;
typedef std::vector<IdxTy> trace_vector_type ;
//typedef _iterator iterator;
_indexed_typeds(): m_pst(0) {}
_indexed_typeds(St & st ): m_pst(&st) {}
void set_pst(St & x) { m_pst=&x; } 
void iter_mode(const IdxTy inseq, const IdxTy outmode)
{
MM_DIE(" no iter_mode code")
}
//iterator begin() { return _iterator(m_map.begin(),m_map,m_vec); } 
//iterator end() { return _iterator(m_map.end(),m_map,m_vec); } 
//iterator find(const IdxTy n ) { return _iterator(m_map.find(n),m_map,m_vec); } 
// NB these references are not good if the vector is updated
// use bracket for direct access, others have indexes
IdxTy find(const IdxTy n ) const  { return Find(n,m_imap); } 
IdxTy find(const StrTy &  n ) const { return Find(n,m_smap); } 

IdxTy find(target_type ** ee, const IdxTy n )   { return Find(ee,n,m_imap); } 
IdxTy find(target_type ** ee, const StrTy n )   { return Find(ee,n,m_smap); } 
target_type & look_up(const IdxTy n) { return m_vec[find(n)]; } 
const target_type & look_up(const IdxTy n) const { return m_vec[find(n)]; } 
target_type & look_up(const StrTy n) { return m_vec[find(n)]; } 
const target_type & look_up(const StrTy n) const { return m_vec[find(n)]; } 

target_type & operator[](const IdxTy n) { return m_vec[n]; } 
const target_type & operator[](const IdxTy n) const  { return m_vec[n]; } 
const IdxTy size() const { return m_vec.size(); }
void trace_alpha(trace_vector_type & v) {Trace(v,m_smap); }  
void trace_alpha_keys(trace_vector_type & v) {TraceKeys(v,m_smap); }  
void trace_numeric(trace_vector_type & v) {Trace(v,m_imap); }  
void trace_numeric_keys(trace_vector_type & v) {TraceKeys(v,m_imap); }  
void add( const IdxTy nidx, const  Tgt & ee) { Add(nidx,ee); } 
IdxTy bad() const { return ~0; } 
private:
template <class Tm,class Tk> IdxTy Find(const Tk &  n, Tm & m  )  const
{ auto ii=m.find(n); if (ii==m.end()) return bad();
return (*ii).second;
}
template <class Tm,class Tk> 
IdxTy Find(target_type  ** ee, const Tk &  n, Tm & m  )  
{ auto ii=m.find(n); if (ii==m.end()) return bad();
*ee=&(m_vec[(*ii).second]);
return (*ii).second;
}
...
//IdxTy find(trget_type * ee, const IdxTy n ) const  { return Find(ee,n,m_imap); } 

void Add( const IdxTy nidx, const  Tgt & ee)  
{
const StrTy units=ee.sfx(); // added as part of index now 
const StrTy & nname=(*m_pst)(nidx);
const StrTy key=StrTy(ee);
const IdxTy ikey=(*m_pst)(key);
auto ii=m_imap.find(ikey);
auto ss=m_smap.find(key);
const bool in_imap=(ii!=m_imap.end());
const bool in_smap=(ss!=m_smap.end());
if (in_imap!=in_smap) { MM_DIE(" index fcked up  "<<MMPR4(units,nname,key,ikey)<<MMPR2(in_imap,in_smap)) } 
IdxTy loc=in_imap?((*ii).second): m_vec.size();
if (!in_imap) 
{ m_imap[ikey]=loc; m_smap[key]=loc; m_vec.push_back(ee);  }
else m_vec[loc].add_raw(ee.qty());
//nmap.add(nidx,ee);
} // Add
template <class Tm> void Trace(trace_vector_type & v, const Tm & m ) 
{
IdxTy i=0; 
MM_LOOP(ii,m) { v[i]=(*ii).second; ++i; }  
} // Trace  
 void TraceKeys(trace_vector_type & v, const StrMap & m ) 
{ IdxTy i=0; MM_LOOP(ii,m) { v[i]=(*m_pst)((*ii).first); ++i; }  } // Trace  
 void TraceKeys(trace_vector_type & v, const IdxMap & m ) 
{ IdxTy i=0; MM_LOOP(ii,m) { v[i]=(*ii).first; ++i; }  } // Trace  
...
typedef _indexed_typeds IndexedTypeds; 
/*
public:
// this has to be strings comes from the caller
typedef std::vector<StrTy> noun_order_type;
...
void dog_daily(Ragged & d, Ragged * used,noun_order_type &no, const IdxTy flags)
...
//static D bad() { return -1; } 
enum {BAD=-1};
public:
snack_data():
//m_time_idx(bad()), m_units_idx(bad()), m_consumer_idx(bad()),
m_noun_idx(bad()), m_time_idx(bad()),  m_consumer_idx(bad()), m_quality_idx(bad()),
//m_qty(-1),m_of(-1),m_pct(-1),m_has_of(false),m_error_idx(0) {}
m_of(BAD),m_pct(BAD),m_has_of(false),m_error_idx(0) {}

snack_data(const StrTy & n, St & st): 
m_noun_idx(st(n)), m_time_idx(bad()),  m_consumer_idx(bad()), m_quality_idx(bad()),
//m_qty(-1),m_of(-1),m_pct(-1),m_has_of(false),m_error_idx(0) {}
m_of(BAD),m_pct(BAD),m_has_of(false),m_error_idx(0) {}

void set_noun(const StrTy & n, St & st) { 
if ( m_eaten.qty()>0) { MM_DIE(" set nound before eaten"); } 
m_noun_idx=st(n); } 
...
void set_eaten(const D & d, const StrTy & sfx)
{if (m_noun_idx==bad()) 
{ MM_DIE(" no noun "); } m_eaten=qty_typed(m_noun_idx,d,(sfx)); }
...
void add_group(const StrTy & g) { m_groups.push_back(g); } 
/*
...
DUMP_DEF((m_eaten.sfx()))  
MM_LOOP(ii,m_groups) { ss<<" "<<(*ii); } 
}
#undef DUMP_DEF
#undef DUMP_DEFN
#undef DUMP_DEFI
...
public:
typedef Tdata data_record_type;
typedef Tdate date_record_type;
...
// FIXME need to track units here or there 
template<class Tps,class Tne,class Tnoun > 
void add(Tps & ps, const Tne & ne, const Tnoun & noun ) { Add(ps,ne,noun); } 
const IdxTy data_size() const { return m_data.size(); } 
const IdxTy dates_size() const { return m_dates.size(); } 
const data_record_type& data(const IdxTy i) const {return m_data[i]; }
const date_record_type& dates(const IdxTy i) const {return m_dates[i]; }
private:
...
// right now really a set 
// each entry is now keyed on noun AND units to allowincompatible
// tallies 
typedef IndexedTypeds  NounMap;
...
{
// TODO this no longer works due to use of thing in units doh... 
StrTy ty=qt.t_u_sfx().unit(); // (*m_pst)(qt.type());
D qty=qt.qty();
MM_ERR(" FCK "<<MMPR2(ty,qty))
(*m_punits).auto_range(qty,ty);
MM_ERR(" FCK OVER "<<MMPR2(ty,qty))
...
l.push_back(ss.str());  
if (include_units){
//const StrTy  ty=qt.sfx(); // (*m_pst)(qt.type());
const StrTy  ty=qt.t_u_sfx().unit(); // (*m_pst)(qt.type());
...
// noun-unit combinations need to be picked to keep columns consistent
// stored in some of these things for various orders
//typedef std::vector<NounTy> NostMap;
// inlucde noun and units 
//typedef std::vector<StrTy> NostMap;
typedef std::vector<IdxTy> NostMap;
...
MM_LOOP(jj,nost)
{
// nmap is a set of qty_dim's and nost is a string. This is stupposed
// to find by suffix. Maybe just index? 
const qty_typed & x=nmap[(*jj)];
//auto ii=nmap.find(*jj);
//if (ii==nmap.end()) continue;
//NO, now ii is noun-unit spec  now ii.second is a vertor
//MM_LOOP(kk,(*ii).second)
//{
Line l;
//AddToLine(l,(*m_pst)((*ii).first),(*ii).second,include_units,auto_range);
AddToLine(l,(*m_pst)(x.noun()),x,include_units,auto_range);
...
void DailyLine(Ragged & d,const DailyMap & m,const NostMap & nostkeys
//, Ulmap &  umap
,NounMap & nmap
...
// the NounMap is the conrived map of typeds 
#if 0
void MakeMaps(NounMap & nmap, Alphanmap & alphanmap, Ulmap & umap)
{
...
const NounTy nidx=(*ii).noun_idx();  
const StrTy & nname=(*m_pst)(nidx);
// no onger makes sense as each noun may have multiple unit accumulators 
#if 0 
// now need a complete index 
auto jj=umap.find(nidx);
if (jj==umap.end()) umap[nidx]=ee.type();
// the old code had to match units but now nmap included units
...
#endif
// the accumulates should never fail now... 
//if (occurance_order)
...
void MakeSomeMaps(NounMap & nmap, NostMap & nost, NostMap & nostkeys, noun_order_type & no, const bool occurance_order)
{
MakeMaps( nmap);
if (no.size()==0)
{
...
nost.resize(nmap.size());
nostkeys.resize(nmap.size());
if (occurance_order)
{  nmap.trace_numeric(nost); nmap.trace_numeric_keys(nostkeys); } 
else {  nmap.trace_alpha(nost); nmap.trace_alpha_keys(nostkeys); } 

//if (occurance_order) nmap.iter_mode(1,0);
//else nmap.iter_mode(0,0);
//MM_LOOP(ii,nmap) nost.push_back((*ii).second); 

}// no.size
else { 
MM_DIE(" this needs to get indicies from nmap ")
//MM_LOOP(ii,no) { nost.push_back((*m_pst)(*ii)); } 
}

} // MakeSomeMaps

void DogDaily(Ragged & d, Ragged * used, noun_order_type &no, const IdxTy flags)
{
...
NounMap  nmap(*m_pst); // this is now the contrived map 
//Ulmap  umap; // no longer needed as each noun entry includes units... 
//Alphanmap alphanmap; // now index is in nmap. 
//MakeMaps( nmap, alphanmap,umap);
NostMap  nost; // this is now  index entries into nmap ... tokenized noun strings 
NostMap  nostkeys; // this is now  index entries into nmap ... tokenized noun strings 
#if 0 
#endif
 MakeSomeMaps( nmap, nost,  nostkeys,  no,  occurance_order);
#if 0
...
const auto ee=nmap[*jj];
 const StrTy f=(*m_pst)(ee.noun());   
 l.push_back(f);   
if (include_units)
{
//const StrTy un=ee.sfx();
...
// this needs a default ctor or something to do this just once doh 
m[c].set_pst(*m_pst);
m[c].add(n,r.eaten());
} // j records for day i 
...
DailyLine(d,m,nostkeys,nmap,push_all,push_zero,labels_each,include_units,
serial,date,fill_units
);
//void DailyLine(Ragged & d,const DailyMap & m,const NostMap & nost
//,const bool push_all,const bool push_zero
...
//Tdata x; x.set_noun(ne.canon(),(*m_pst));
Tdata x(ne.canon(),(*m_pst));

const auto & gm=noun.groups();
MM_LOOP(ii,gm) { x.add_group((*ii).first); } 
//m_data.push_back(x);
