...
{
// the block_descend code does not work but 
// the Working code should doe the same thing. 
// 
const bool block_descend=!true;
if ( true) { NewChLatexWorking(c); return; } 
...
if (BlockStart(c))
{
	...
	if ( Kidding()) { //  
		...
		EndKid();  
		NewChildren( kid,  c,2); // now just 1 deep 
		} else {
		StrTy kid= m_rdbuf.mark_string(); 
		EndKid();  
	...
	// otherwise just another piece of text 
		...
		m_rdbuf.start_new(); 
	//	Blank();
		NewChildren( "", c,2-1); // now just 1 deep 
		} else {  
		Blank();
...
if (BlockEnd(c)) 
{
	...
	//	RemoveBack();// m_stack->back()->NewId(c); 
		Myt* p=m_stack->back();
	 	p->m_rdbuf.new_append(c); 
		p->RemoveBack();// m_stack->back()->NewId(c); 
		p=m_stack->back();
//		p->m_rdbuf.start_new();  
	//	p->Blank();  
		return; 
		...
		// this adds the delminter to the kid thought, not this
		//NewChildren( kid,  c,1);
		NewChildren( kid,  1);
		RemoveBack();// m_stack->back()->NewId(c); 
		m_rdbuf.start_new();  
	 	m_rdbuf.append(c); 
		Blank();  
		return; } //  !block_descend
		 } // Kidding() 
	else { 
		if (block_descend)
		{
		Myt* p=m_stack->back();
		p->m_rdbuf.append(c); // p->m_rdbuf.start_new();  p->Blank(); 
//		 p->m_rdbuf.start_new(); 
		RemoveBack();	
		 p=m_stack->back();
//		 p->m_rdbuf.start_new(); 
		...
		m_rdbuf.append(c); m_rdbuf.start_new();  Blank(); return;   
		} // !block_descend
...
if (Eol(c)) { 
if (Kidding()) {NewId(c);  
Reset(EOL_RESET|FIRST_ON); 
if (block_descend) RemoveBack(); 
return;  }
Reset(EOL_RESET); 
}
...
if (Kidding())
{
if (IdEnd(c)) { NewId(c); if (!block_descend) RemoveBack();

//if (block_descend) m_stack->back()->Blank(); RemoveBack(); 
...
if (m_latex_mode) if (m_tag=="\\end") ss<<m_rdbuf.string(0);
if (nest&&show_tag&&!full&&!(m_tag.length()==0)) ss<<CRLF;
}
 }
{ // scoping sz wtf
const IdxTy sz=ss.str().length();
...
bool Visible(){return   ((m_disp_level=="1")||(m_disp_level=="all")); } 
void CycleDisp()
{
...
if (cmd=="next" ) // make the first invisible one visible now 
{
tgt=this;
if (!tgt->Visible()) { tgt->On(); return 1 ; } 
IdxTy sz=tgt->m_family.size();
for(IdxTy i=0; i<sz; ++i) {if ( tgt->m_family[i]->Cmd(cmd,true)==1) return 1;  }
return 0;
}
...
void NewChildren(const StrTy & tag, const IdxTy n)
{
//MM_ERR(" making kid ")
Myt * kid = NULL;
StrTy t=tag;
...
StrVars m_svars;
IntVars m_ivars;
// children display interspersed with strings in m_rdbuf
Children m_kids ;
Parent m_mom;
ChildStack* m_stack;
ChildStack m_family;
...
IdxTy m_parse_state;
// these contain text strings associated with this block 
RdBuf m_rdbuf;
bool m_latex_mode;
