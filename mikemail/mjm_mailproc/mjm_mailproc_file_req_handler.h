#ifndef MJM_MAILPROC_FILE_REQ_HANDLER_H__
#define MJM_MAILPROC_FILE_REQ_HANDLER_H__
...
template <class Tr> class mjm_mailproc_file_req_handler : public mjm_mailproc_handler<Tr>
{
 typedef mjm_mailproc_handler<Tr> Super;
 typedef mjm_mailproc_file_req_handler<Tr> Myt;
...
// for registration 
mjm_mailproc_file_req_handler(const StrTy & nm ) {Super::ptr::name(nm,&Myt::factory());    }
mjm_mailproc_file_req_handler(const StrTy & nm,const ctor_vars & cv ) 
{Super::ptr::name(nm,&Myt::factory(cv));    }
mjm_mailproc_file_req_handler() {}
mjm_mailproc_file_req_handler(const ctor_vars & cv ) {Init(cv);}
virtual ~mjm_mailproc_file_req_handler() {}
virtual Myt &  operator()() { return *this; } 
//  static, NOT virtual  need an instance of the derived class 
static  Super * factory() { return  new mjm_mailproc_file_req_handler(); } 
static  Super * factoryc(const ctor_vars & cv) 
{ return  new mjm_mailproc_file_req_handler(cv); } 
...
{ const auto & l=r.line(i); if (l.size()==0) continue;
MM_ERR(" adding "<<l[0]);
//attlist[j]=(char*)l[0].c_str(); ++j;
attlistfck.push_back(l[0]);
}
attlist[j]=0;
p->reply(ss.str(),StrTy("stuff here "),attlistfck,mc);
delete [] attlist;
} // p!=0
} // scoping ss
/*
