...
#include "mjm_toobib_diary_form.h"
...
typedef mjm_toobib_diary_form<Tr> ToobibForm;

typedef mjm_global_mailusers<Tr> Users;
typedef typename Users::user_desc Ud;


typedef typename ToobibForm::parse_state PS;
typedef typename ToobibForm::parse_settings PO;
...
mjm_mailproc_toobib_handler() {}
mjm_mailproc_toobib_handler(const ctor_vars & cv ) {Init(cv);}
virtual ~mjm_mailproc_toobib_handler() {}
...
// toobib {template,submit,commit,download,report } [filename]
...
//if (n>(2+base)) dv.add("_file",sub[2+base]); else dv.add("_file","toobib.txt");
dv.add("_file","toobib.txt");
...
const StrTy su= (m.hval_lc("Subject:"));
Ss ss;
ss<<" the message you sent appears to want action on a toobib docuenet"<<CRLF;
...
IdxTy ReloadForm(ToobibForm & df, dispatch_vars & dv,const IdxTy flags=0) { 
//MM_ERR("HandleAppend")
